...
The DOS templating system enables **prompts that write prompts**—dynamic composition where structure is fixed but content is parameterized. Based on Anthropic's official `{{variable}}` syntax and industry best practices.
...
rm -rf Templates/
git checkout v2.5.0 -- DOS/Prompting.md
```
...
- `~/.claude/DOS/Prompting.md` (Templating section)
