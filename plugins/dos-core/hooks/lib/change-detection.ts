/**
 * change-detection.ts - Utilities for detecting DOS system changes
 *
...
const STATE_FILE = join(DOS_DIR, 'MEMORY', 'STATE', 'integrity-state.json');
...
/**
 * Normalize an absolute path to relative (to DOS_DIR).
 */
function normalizeToRelativePath(absolutePath: string): string {
  if (absolutePath.startsWith(DOS_DIR)) {
    return relative(DOS_DIR, absolutePath);
  }
  return absolutePath;
}
...
/**
 * Categorize a file path by its location in the DOS system.
 */
export function categorizeChange(path: string): ChangeCategory | null {
  ...
  // Check if path is within DOS directory
  const absolutePath = path.startsWith('/') ? path : join(DOS_DIR, path);
  if (!absolutePath.startsWith(DOS_DIR)) {
    return null;
  }
  ...
  if (path.includes('skills/')) {
    ...
    if (path.match(/DOS\/(?:PAISYSTEM|THEHOOKSYSTEM|THEDELEGATION|MEMORYSYSTEM|AISTEERINGRULES)/)) return 'core-system';
    return 'skill';
  }
...
export function isSignificantChange(changes: FileChange[]): boolean {
  // Filter to only DOS system changes
...
export function generateDescriptiveTitle(changes: FileChange[]): string {
  ...
  for (const p of paths) {
    ...
    if (match && match[1] !== 'DOS') skillNames.add(match[1]);
  }
  ...
  const hasCoreSystem = paths.some(p => p.match(/DOS\/(?:PAISYSTEM|THEHOOKSYSTEM|THEDELEGATION|MEMORYSYSTEM|AISTEERINGRULES)/));
  const hasCoreUser = paths.some(p => p.includes('DOS/USER/'));
  ...
  // Core system changes
    ...
    const docNames = paths
      .filter(p => p.match(/DOS\/(?:PAISYSTEM|THEHOOKSYSTEM|THEDELEGATION|MEMORYSYSTEM|AISTEERINGRULES)/))
      .map(p => basename(p, '.md'));
    if (docNames.length === 1) {
      title = `${docNames[0]} Documentation Updated`;
    } else {
      title = 'DOS System Documentation Updated';
    }
  }
  // Core user changes
  else if (hasCoreUser) {
    const docNames = paths
      .filter(p => p.includes('DOS/USER/'))
      .map(p => basename(p, '.md'));
    if (docNames.length === 1) {
