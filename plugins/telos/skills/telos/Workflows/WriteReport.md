---
name: Write Report
description: Generates a McKinsey-style professional consulting report as a dark-themed Next.js web document from TELOS assessment artifacts, with automatic light-mode print support.
status: STABLE
bestPath:
  - title: "Analyze"
    description: "Verify the assessment workflow has produced the artifacts/*.json contract files."
  - title: "Narrative"
    description: "Ensure CreateNarrativePoints has generated narrative.json story content."
  - title: "Structure"
    description: "Copy the report template and generate report-data.ts from the artifacts."
  - title: "Render"
...
- User says "write report", "McKinsey report", "create TELOS report", or "professional report"
- A TELOS assessment (findings, narrative, recommendations) already exists and needs a board- or client-ready rendered deliverable
- Prerequisite: CreateNarrativePoints must run first to generate the narrative content this workflow maps into report sections
...
The assembly is a deterministic transform — read the five `artifacts/*.json` files,
map them into the fixed `ReportData` schema (with the documented fallback chains:
`keyFindings` falls back to the first five finding titles, `primaryRecommendation`
to the first recommendation's description), and emit the `lib/report-data.ts`
source. That transform is owned by `renderReportData()` in
`Telos/src/Tools/TelosRender.ts` (golden-tested for byte-identical output). Do NOT
hand-assemble the object in prose — invoke the CLI subcommand so the schema and
fallbacks stay pinned in code:

```bash
bun ~/.claude/skills/telos/Tools/TelosRender.ts render-report-data \
  "{source}/artifacts" \
  --client "{client_name}" \
  --title "Strategic Assessment & Transformation Roadmap" \
  --date "December 2025" \
  --classification "CONFIDENTIAL" \
  > "{output_dir}/lib/report-data.ts"
```

The subcommand reads `{source}/artifacts/{findings,recommendations,roadmap,methodology,narrative}.json`
and writes the assembled `lib/report-data.ts` to stdout. The `--client`, `--title`,
`--date`, and `--classification` flags are the per-engagement meta fields; the rest
of the report content comes deterministically from the artifacts.
