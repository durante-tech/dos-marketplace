...
interface WorkMeta {
  ...
  title: string;
  created_at: string;
  /** Legacy META.yaml field; preserved for back-compat with existing files.
   * V13.7-β migration (2026-05-13): may be `null` for vNext PRDs after the
   * SessionCleanup retirement that follows V14.1; duration falls back to
   * `updated` then. */
  completed_at: string | null;
  /** vNext PRD frontmatter field (RFC-0080 §2.2). Reflects the latest
   * mutation timestamp; used as the V13.7-β fallback when `completed_at`
   * is absent. */
  updated?: string | null;
  source: string;
  status: string;
...
function writeLearning(workMeta: WorkMeta, idealContent: string): void {
  ...
  // V13.7-β migration (2026-05-13): prefer `completed_at` (legacy META.yaml),
  // fall back to `updated` (vNext PRD frontmatter). This unblocks v0.0.14
  // retirement of the SessionCleanup `completed_at` write while preserving
  // back-compat with existing META.yaml files in MEMORY/WORK/.
  let duration = 'Unknown';
  const endStamp = workMeta.completed_at ?? workMeta.updated ?? null;
  if (workMeta.created_at && endStamp) {
...
const _t = startTimer('WorkCompletionLearning');
