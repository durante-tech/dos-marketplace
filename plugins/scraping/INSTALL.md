...
elif [ -f "$HOME/Projects/DOS/.env" ] && grep -q "APIFY_TOKEN" "$HOME/Projects/DOS/.env" 2>/dev/null; then
  echo "OK Apify API token found in DOS .env"
else
  echo "INFO Apify API token not found (needed for social media and platform scraping)"
fi

# Check for Bright Data code-first client
if [ -f "$CLAUDE_DIR/mcp.json" ] 2>/dev/null; then
  if grep -q -i "bright" "$CLAUDE_DIR/mcp.json" 2>/dev/null; then
    echo "OK Bright Data code-first client configured"
  else
    echo "INFO Bright Data code-first client not configured (Tier 4 proxy scraping unavailable)"
  fi
else
...
if [ -d "$CLAUDE_DIR/DOS/USER/SKILLCUSTOMIZATIONS/BrightData" ]; then
  echo "OK BrightData customizations found"
else
  echo "INFO No BrightData customizations (skill will use defaults)"
fi

if [ -d "$CLAUDE_DIR/DOS/USER/SKILLCUSTOMIZATIONS/Apify" ]; then
...
- Bright Data code-first client: [configured / not configured -- needed for Tier 4 proxy]
...
[If Bright Data code-first client not configured]: Note: Tier 4 proxy scraping (for CAPTCHA and advanced bot detection)
requires the Bright Data code-first client. Tiers 1-3 work without it."
```
...
Tier 4 (Bright Data proxy with CAPTCHA solving) requires the Bright Data code-first client.
...
If Tiers 1-3 fail and Tier 4 is not configured, set up the Bright Data code-first client in your Claude Code configuration for professional proxy scraping.

### Apify: "APIFY_TOKEN not set"

Get your API token from https://console.apify.com/account/integrations and set it as `APIFY_TOKEN` in your environment or DOS .env file.
