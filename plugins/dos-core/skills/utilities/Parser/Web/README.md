...
**Honesty indicators (so mock output is never mistaken for real parses):**
- A persistent "DEMO MODE" banner renders at the top of the results section.
- Every result card shows a "DEMO / MOCK" badge.
- Each emitted record carries `extraction_metadata.demo_mode: true`, `processing_method: "mock-demo"`, and a `warnings[]` entry stating the record is synthetic.

The per-card badge is gated on `demo_mode === true`, so once a real backend is wired (and stops setting that flag), the badge disappears automatically.
