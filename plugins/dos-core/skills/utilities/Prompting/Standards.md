...
## Native Fabric Patterns in DOS

**Location:** `~/.claude/skills/utilities/Fabric/Patterns/`

DOS maintains a local copy of all Fabric patterns for native execution. Instead of spawning the `fabric` CLI for every pattern-based task, the system reads and applies patterns directly as prompts.
...
Sync latest patterns via the Fabric UpdatePatterns workflow: Read
`~/.claude/skills/utilities/Fabric/Workflows/UpdatePatterns.md` and follow it.

This pulls upstream updates via `fabric -U` and syncs to DOS's local copy.
...
**Foundation:** Based on Anthropic's official `{{variable}}` syntax, industry patterns (LangChain, Handlebars, DSPy), and DOS's unique skill architecture.

**Philosophy:** Templates enable prompts to write prompts—dynamic composition where structure is fixed but content is parameterized. This is core DOS DNA.

---

## Core Syntax

DOS uses Handlebars notation for template variables:
