...
- **Art workflows:** 20 (AnnotatedScreenshots, Aphorisms, Comics, Comparisons, CreateDOSPackIcon, D3Dashboards, Essay, Frameworks, Maps, Mermaid, RecipeCards, RemoveBackground, Stats, Taxonomies, TechnicalDiagrams, Timelines, Visualize, YouTubeThumbnailChecklist, AdHocYouTubeThumbnail, BrandWallpaper)
...
| "create a DOS pack icon for the research skill" | Routes to Art > CreateDOSPackIcon workflow |
...
- **API keys** for at least one image generation backend, stored in `${DOS_DIR}/.env`
- **bun** runtime for executing TypeScript tools

### Optional

- **User customizations** at `~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/Art/` for aesthetic preferences, character specs, and scene construction guidelines
- **Remotion customizations** at `~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/Remotion/`
...
Create `~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/Art/PREFERENCES.md` to define:
...
- **Original concept:** Daniel Miessler -- developed as part of the [PAI](https://github.com/durante-tech/dos) system
...
- **DOS Art Skill** -- The image generation engine at the core of the media pack
- **DOS Remotion Skill** -- Programmatic video creation with React
- **DOS Blogging Skill** -- Generates blog posts that benefit from Media-created header images
...
- **DOS Algorithm** -- Algorithm-driven work sessions can generate visual artifacts inline
