...
description: Compare prompt variants on the same eval use case to find the strongest.
status: STABLE
---
...
```bash
bash "$DOS_DIR/DOS/Tools/voice.sh" main "Running the ComparePrompts workflow in the Evals skill to A/B test prompts"
```
...
Render each prompt version (RenderTemplate, Step 2), run each against the same test inputs
(the agent itself, or per-model via the openrouter skill), and write one
`Results/<use-case>/<run-id>/results.json` per version. Grade both with the use case's scorers,
applying the Position Swapping Protocol below to judge-graded pairs.
