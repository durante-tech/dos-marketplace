...
## Subagent Algorithm Profile (applies to BOTH systems)

Every spawned subagent — whether via `Task` with a ComposeAgent prompt (custom agent) OR via `TeamCreate` → team-member dispatch — runs the **Subagent Algorithm Profile** maintained in `~/.claude/DOS/PARTIALS/_algorithm-lite.md` and injected by the active Algorithm runtime.

**For `TeamCreate` briefs specifically:** include (or reference) the profile content from `~/.claude/DOS/PARTIALS/_algorithm-lite.md` in the team-member system prompt. Without this, team members silently drop to NATIVE mode and complete with zero tool uses — the exact failure mode this profile was designed to prevent.

Spawned members **SKIP**: voice curls (primary-only per RFC-0016 §7.2 Option B), PRD writes, reflection JSONL, effort classification, Council-from-Council, ScheduleWakeup.

Spawned members **EXECUTE**: OBSERVE-lite parallel batch (`🔎 FILES TO READ` / `🧠 MEMORY TO RECALL` / `🏹 CAPABILITIES`), BUILD capability invocation, VERIFY capability-invocation check.

**MAX_RECURSION: 2.** A team member MAY spawn one level of sub-subagent; deeper nesting is forbidden — the member must return its task up to the parent for re-dispatch.

Zero-tool-use completion on a capability-naming team member = FAILURE. The parent should reject the result and re-dispatch with tighter capability guidance.
