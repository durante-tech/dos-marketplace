---
name: Fabric
description: Route content through the 242+ Fabric pattern library (extract_wisdom, summarize, analyze_claims, ...) via the fabric CLI
status: STABLE
bestPath:
  - title: "Pick Pattern"
    description: "Match user intent to a Fabric pattern (fabric --listpatterns for the inventory)."
  - title: "Execute"
    description: "Run fabric -p <pattern> over the URL/text/transcript input."
  - title: "Deliver"
    description: "Return the pattern output; save to the vault when it is research-grade."
---
...
- User names a Fabric pattern or says "use fabric"
- A specialized prompt pattern fits better than freeform inference (extract_wisdom, analyze_claims, ...)
- YouTube inputs route via YoutubeExtraction (which shells fabric -y)
