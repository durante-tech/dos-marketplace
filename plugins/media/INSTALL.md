...
## Studio Gateway Mode (Optional)

The media pack supports two modes for Replicate-backed generations:

**BYOK mode (default — unchanged):** set `REPLICATE_API_TOKEN` in `${DOS_DIR}/.env`. Tools call Replicate directly with your key, you pay Replicate.

**Gateway mode (opt-in):** set `STUDIO_API_URL` and `STUDIO_API_KEY` in `${DOS_DIR}/.env`. Tools route through Durante Studio, which handles metering, caching, and credit-based billing with a pooled Replicate account. You pay Studio in credits; the `REPLICATE_API_TOKEN` variable becomes optional and is ignored when gateway mode is active.
...
Remove or comment `REPLICATE_API_TOKEN` — it's no longer required.

The switch is invisible to callers: the shim at `Packs/media/src/Lib/gateway.ts` is a drop-in replacement for the Replicate SDK's `Replicate` class, so all 9 Replicate-backed tools (Art, Video, Speech, Music, ImageUpscale, ImageRestoration, BackgroundRemoval, FaceSwap, Emoji) auto-route through Studio when the two env vars are set, and fall back to direct Replicate otherwise.
