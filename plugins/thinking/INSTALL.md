...
# Check for DOS infrastructure (optional)
if [ -d "$CLAUDE_DIR/DOS" ]; then
  echo "OK DOS directory exists (full integration available)"
else
  echo "INFO DOS directory not found (skill will work standalone)"
fi
...
- DOS infrastructure: [found (full integration) / not found (standalone mode)]
