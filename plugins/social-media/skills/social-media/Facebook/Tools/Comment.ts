...
/**
 * Comment — Reply to or delete a comment on a Facebook Page post.
 *
 * Actions:
 ...
 *
 * Usage:
 *   bun Comment.ts --action reply --comment-id 123_456 --message "Thanks!"
 *   bun Comment.ts --action delete --comment-id 123_456 --yes
 */
...
function showHelp(): void {
  console.log(`
Comment - Facebook Comment Reply/Delete CLI (Graph API ${GRAPH_VERSION})

Reply to or delete a comment on a Facebook Page post.
...
async function main(): Promise<void> {
  try {
    ...
    if (!pageToken) throw new CLIError("Missing FACEBOOK_PAGE_TOKEN. Run Login.ts first.");

    if (args.action === "reply") {
      console.log(`💬 Replying to comment ${args.commentId}...`);
      const response = await graph<{ id: string }>(`/${args.commentId}/comments`, {
        method: "POST",
        token: pageToken,
      ...
      console.log(`🗑️  Deleting comment ${args.commentId}...`);
