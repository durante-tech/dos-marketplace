...
description: Define a new eval use case with inputs and expected behavior.
status: STABLE
---
...
\`\`\`bash
bun run ~/.claude/skills/utilities/Evals/Tools/AlgorithmBridge.ts -s <suite-containing-this-use-case>
\`\`\`
...
# Validate config parses + suite membership
...
```bash
# Add the use case to a suite, then run it once to verify setup
bun run ~/.claude/skills/utilities/Evals/Tools/SuiteManager.ts add-task <suite> <name>
bun run ~/.claude/skills/utilities/Evals/Tools/AlgorithmBridge.ts -s <suite>

# Multi-trial a single task file (pass@k infra)
