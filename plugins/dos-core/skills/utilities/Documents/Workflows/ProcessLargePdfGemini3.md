---
name: Process Large Pdf Gemini3
description: Process and extract content from a large PDF using Gemini 3 Pro.
status: STABLE
---
...
The `document.parse/generate` Port at `Tools/dos-doc.ts` documents the routing surface for document operations:

```bash
# Parse text from PDF — Port routes to Pdf Adapter (stubbed Phase 2.A)
bun Tools/dos-doc.ts /path/to/research.pdf --intent=parse-text \
  --telemetry-tag=Documents/ProcessLargePdfGemini3

# Parse tables from XLSX — Port routes to Xlsx Adapter (stubbed)
bun Tools/dos-doc.ts /path/to/data.xlsx --intent=parse-tables

# Generate PDF — Port routes to Pdf Adapter (stubbed)
bun Tools/dos-doc.ts /tmp/draft.md --intent=generate-pdf \
  --output=~/Downloads/draft.pdf
```

**Phase 2.A status:** all 4 Adapters (Docx / Pdf / Pptx / Xlsx) currently throw `[Phase 2.B] X Adapter not yet wired`. The walking skeleton ships the routing geometry + intent surface + telemetry; Phase 2.B will wire per-format entrypoints from the official document-skills plugin (`~/.claude/plugins/marketplaces/anthropic-agent-skills/skills/{format}/`) — the vendored `Utilities/Documents/{Format}/Scripts/` trees were retired 2026-07-11 (Prospector Gen 40).

Use the Port form preview when:
- Documenting future intent-driven document workflows
- Designing skill calls that will activate when Phase 2.B lands
- Exploring routing decisions via test fixtures (the pure-function `decideDocAdapter` is stable today)

Use this workflow (Gemini 3 multimodal) when:
- The actual PDF processing TODAY (no waiting for Phase 2.B)
- Multimodal extraction (text + images + tables) in a single LLM call
- Working with PDFs that exceed traditional parser capacity

---

## Overview

This workflow uses Gemini 3 Pro's native multimodal capabilities to process large, complex PDFs that contain mixed content (text, images, tables, charts, diagrams). Unlike traditional PDF extraction tools that require conversion and lose context, Gemini 3 Pro processes PDFs directly as multimodal input.
...
- For traditional PDF extraction (text-only), see the official document-skills plugin's `pdf` skill (`~/.claude/plugins/marketplaces/anthropic-agent-skills/skills/pdf/SKILL.md`)
- For PDF form filling, see the plugin's `pdf` skill forms reference (`~/.claude/plugins/marketplaces/anthropic-agent-skills/skills/pdf/forms.md`)
