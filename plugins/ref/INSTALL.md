# Ref v1.0.0 - Installation Guide

This guide is for AI agents installing the ref pack into a DOS-compatible `~/.claude` tree.
...
- Studio has Ref pricing rows and `STUDIO_POOL_REF_API_KEY` configured.
...
if [ -d "$CLAUDE_DIR/skills/Ref" ]; then
  echo "WARNING Existing ref skill found at $CLAUDE_DIR/skills/Ref"
else
  echo "OK Clean Ref install target"
fi
```
...
```bash
CLAUDE_DIR="$HOME/.claude"
BACKUP_DIR="$CLAUDE_DIR/Backups/ref-skill-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP_DIR"
[ -d "$CLAUDE_DIR/skills/Ref" ] && cp -R "$CLAUDE_DIR/skills/Ref" "$BACKUP_DIR/Ref"
