---
name: Create Judge
description: Create an LLM judge that scores eval outputs against defined criteria.
status: STABLE
---
...
```bash
bun run ~/.claude/skills/utilities/Evals/Tools/TrialRunner.ts -t UseCases/<domain>/<name>/task.yaml -n 1
# then inspect the judge's output in the newest Results/<use-case>/<run-id>/results.json
```
