...
| Web Articles | Generic HTML | `Research.searchPerspectives` (Gemini backend) full scraping |
...
### With the Research Intent API

The parser routes entity-extraction work through the intent-verb API from
RFC-0015 §6 — `Research.searchPerspectives` with `providers: ["gemini"]` —
so the heavy lifting runs behind the Studio gateway and inherits cache,
credit reservation, and rate-limit guardrails:

```typescript
import { Research } from "Packs/research/src";

await Research.searchPerspectives(
  `Extract entities (people, companies, topics, links) from: [content]`,
  { count: 1, providers: ["gemini"], diversity: "low", depth: "deep" },
);
```
...
- [ ] Add Twitter scraping (Apify code-first client)
...
Private - Part of DOS infrastructure
