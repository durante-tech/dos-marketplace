 ...
 *   US_METRICS_DATASET - Dataset directory (default: ~/Projects/Substrate/Data/US-Common-Metrics)
 *
 * Output files:
...
// Dataset root resolution (F1): an explicit US_METRICS_DATASET env override wins;
// absent that we fall back to the conventional ~/Projects/Substrate layout. main()
// emits an actionable error when the resolved path is missing, so a customer install
// without the operator's directory layout no longer hard-fails on a baked-in path.
const DEFAULT_SUBSTRATE_PATH = join(process.env.HOME || "", "Projects/Substrate/Data/US-Common-Metrics");
const SUBSTRATE_PATH = process.env.US_METRICS_DATASET?.trim() || DEFAULT_SUBSTRATE_PATH;
...
`);
  ...
  if (!existsSync(SUBSTRATE_PATH)) {
    console.error(`Error: dataset path not found: ${SUBSTRATE_PATH}`);
    console.error(
      process.env.US_METRICS_DATASET
        ? "US_METRICS_DATASET is set but points at a missing directory — create it or fix the path."
        : "Set US_METRICS_DATASET to your US-Common-Metrics dataset directory (or create the default path shown above).",
    );
    process.exit(1);
  }
  ...
  await writeArtifact(mdPath, updatedMd, {
    pack: "USMetrics",
    workflow: "UpdateSubstrateMetrics",
    artifactType: "markdown",
    title: "US-Common-Metrics.md",
  });
  console.log(`  ✓ ${mdPath}`);

  await writeArtifact(currentCsvPath, currentCsv, {
    pack: "USMetrics",
    workflow: "UpdateSubstrateMetrics",
    artifactType: "csv",
    title: "us-metrics-current.csv",
  });
  console.log(`  ✓ ${currentCsvPath}`);

  // Append to historical (create header if new file). Header write goes through
  // writeArtifact (one-time create); the recurring append uses appendFileSync
  // because writeArtifact replaces rather than appends — append-only growth is
  // the SCRATCH/JSONL pattern explicitly exempted by writeArtifact's docstring.
  if (!existsSync(historicalCsvPath)) {
    await writeArtifact(historicalCsvPath, "fetch_timestamp,metric_id,value,period\n", {
      pack: "USMetrics",
      workflow: "UpdateSubstrateMetrics",
      artifactType: "csv",
      title: "us-metrics-historical.csv (header)",
    });
  }
  // writeArtifact:exempt — append-only historical CSV growth (writeArtifact replaces; this needs append)
