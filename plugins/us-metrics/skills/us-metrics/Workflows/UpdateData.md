---
name: Update Data
description: Fetch current data from all sources and update the Substrate US-Common-Metrics dataset.
status: STABLE
bestPath:
  - title: "Run Update Tool"
    description: "Execute UpdateSubstrateMetrics.ts to fetch live values from FRED, EIA, and Treasury APIs."
  - title: "Write Substrate Files"
    description: "Populate US-Common-Metrics.md and us-metrics-current.csv, and append to us-metrics-historical.csv."
  - title: "Verify Update"
    description: "Confirm values are current (not placeholders) and check the update log for failed fetches."
divergence_from_canonical:
  _intent-to-flag-table.md:
    partial_version: 1.0.0
    reason: "USMetrics UpdateData workflow uses workflow-specific intent-to-flag vocabulary; canonical Mode/Output two-table shape doesn't fit"
  _workflow-output-shape.md:
    partial_version: 1.0.0
    reason: "USMetrics UpdateData workflow has bespoke Output section with workflow-specific shape"
---
...
- User says "update the metrics", "refresh data", "pull latest", "update Substrate dataset"
- Must run before GetCurrentState to ensure the analysis reads current values
- NOT for generating the trend-analysis report itself (use GetCurrentState) — this workflow only fetches and writes data
...
This workflow shells out to `Tools/UpdateSubstrateMetrics.ts`. Translate operator phrasing into deterministic flag selection per CreateSkill workflow Step 6 + CliFirstArchitecture.md.

### Mode Selection

| User Says | Flag | Effect |
|-----------|------|--------|
| "update the metrics", "refresh data", "pull latest" (default) | (none) | Fetches all configured series from FRED, EIA, Treasury and writes Substrate files |
| "preview the update", "dry run", "what would change without writing" | `--dry-run` | Fetches data but does NOT write `US-Common-Metrics.md`, `us-metrics-current.csv`, or append to `us-metrics-historical.csv` |
