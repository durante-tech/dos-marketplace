---
name: Create DOS Pack Icon
description: 
status: STABLE
---

# DOS Pack Icon Workflow
...
3. **Consistent style** - Match existing DOS pack icons
...
**Good icon concepts:**
- `dos-hook-system` → Hook shape, event trigger
- `dos-core-install` → Download/install arrow
...
bun run ~/.claude/skills/media/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/DOS/Packs/icons/[PACK_NAME].png
```
...
ls -la ${PROJECTS_DIR}/DOS/Packs/icons/[PACK_NAME].png

# Check dimensions (requires imagemagick)
file ${PROJECTS_DIR}/DOS/Packs/icons/[PACK_NAME].png
```
...
bun run ~/.claude/skills/media/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/DOS/Packs/icons/dos-hook-system.png
```
...
bun run ~/.claude/skills/media/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/DOS/Packs/icons/dos-core-install.png
```
...
bun run ~/.claude/skills/media/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/DOS/Packs/icons/dos-memory-system.png
```
...
All DOS pack icons go to:
```
${PROJECTS_DIR}/DOS/Packs/icons/[PACK_NAME].png
```

**Naming convention:** Match the pack directory name exactly.
- Pack: `Packs/dos-hook-system/`
- Icon: `Packs/icons/dos-hook-system.png`
...
- [ ] **Exists** at `${PROJECTS_DIR}/DOS/Packs/icons/[PACK_NAME].png`
...
- [ ] **Consistent** - matches other DOS icons in style

---

## Related Workflows

- `the DOS release workflow` - Release workflow (may include icon generation)

*Note: Previously referenced CreatePack.md, ValidatePack.md, and DOSIntegrityCheck.md have been removed.*
