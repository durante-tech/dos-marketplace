...
if [ -n "$APIFY_TOKEN" ]; then
  ...
  echo "  AVAILABLE Apify API token (DOS .env)"
else
  echo "  UNAVAILABLE Apify API token (get from https://console.apify.com/account/integrations)"
fi

# Bright Data code-first client
CLAUDE_DIR="$HOME/.claude"
if [ -f "$CLAUDE_DIR/mcp.json" ] && grep -q -i "bright" "$CLAUDE_DIR/mcp.json" 2>/dev/null; then
  echo "  AVAILABLE Bright Data code-first client (Tier 4 proxy scraping)"
else
  echo "  UNAVAILABLE Bright Data code-first client (Tier 4 proxy scraping unavailable)"
fi
...
- [ ] Bright Data code-first client configured (for Tier 4)
