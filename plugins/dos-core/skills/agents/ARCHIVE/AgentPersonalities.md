...
**Canonical source of truth for all DOS agent personality definitions.**

This file defines the character, voice settings, backstories, and personality traits for all agents in the DOS system. The voice server reads this configuration to deliver personality-driven voice communication.

## Hybrid Agent Model

DOS uses a **hybrid agent system** that combines:
...
4. Test with: `bash "$DOS_DIR/DOS/Tools/voice.sh" custom --voice-id VOICE_ID "Test"`
...
- **v1.3.0** (2025-11-16): Centralized in DOS, increased expressiveness for all agents
