...
`~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/Apify/`
...
## ⚡ Quick CLI (single Bash call — METERED via Studio)

For one-shot actor runs, prefer the CLI over inline TypeScript. Routes through `/api/v1/scraping/apify/run` so every call is metered:

```bash
bun ~/.claude/skills/scraping/Apify/Tools/RunApify.ts <actorId> --input <json>
```
...
bun ~/.claude/skills/scraping/Apify/Tools/RunApify.ts apify/instagram-scraper \
  --input '{"usernames":["nasa"],"resultsLimit":10}'

# Google Maps lead generation
bun ~/.claude/skills/scraping/Apify/Tools/RunApify.ts apify/google-maps-scraper \
  --input '{"searchStringsArray":["restaurants in San Francisco"],"maxCrawledPlaces":50}'

# Read input from a file (for actors with complex JSON schemas)
bun ~/.claude/skills/scraping/Apify/Tools/RunApify.ts apify/web-scraper \
  --input-file ./web-scraper-input.json --json-only
```

**Output:** Always JSON on stdout: `{ actorId, items, itemCount, chargedCredits, mode, generationId }`. Pipe to `jq` for filtering. Use `--json-only` to suppress stderr status lines for clean piping.

**When to skip the CLI** and use the TypeScript class directly: when you need `getDataset(...)`-style streaming/pagination, or when the actor's run is one step in a larger TypeScript script that filters or aggregates downstream.
...
**Architecture:** See `~/.claude/DOS/DOCUMENTATION/FileBasedMCPs.md`
