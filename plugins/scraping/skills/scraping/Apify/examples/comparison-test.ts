...
import { getApifyClient } from "@durante/scraping/Apify"
...
async function demonstrateMCPApproach() {
  ...
  console.log('Step 1: search-actors (legacy MCP)')
...
async function demonstrateCodeFirstApproach() {
  ...
  const apify = await getApifyClient()
  ...
  const codeExample = `
import { getApifyClient } from '~/.claude/filesystem-mcps/apify'
