...
export class ToolCallVerificationGrader extends BaseGrader {
  ...
  async grade(context: GraderContext): Promise<GraderResult> {
    ...
    // Empty/mistyped params build zero checks — `0 === 0` would score 1.0 and
    // report passed. Fail closed instead (mirror BinaryTests.ts / RegexMatch.ts).
    if (checks.length === 0) {
      return this.createResult(0, false, performance.now() - start, {
        reasoning: 'No tool call checks configured',
      });
    }
