...
description: Architecture and system design visuals with Excalidraw aesthetic
status: STABLE
featured: true
successRate: 95.1
icon: BarChart3
bestPath:
  - title: "System Analysis"
    description: "Parse system architecture and identify key components and relationships."
  - title: "Component Mapping"
    description: "Map services, data flows, and dependencies into a structured graph."
  - title: "Visual Layout"
    description: "Arrange components with optimal spacing, grouping, and hierarchy."
  - title: "Excalidraw Render"
    description: "Render final diagram with drafted technical Excalidraw aesthetic and annotations."
---
...
- Color: Blueprint Navy #1E3A8A or Black #000000
...
- Color: Hot Accent Orange #F97316 (primary) or Blueprint Navy #1E3A8A
...
Hot Accent Orange #F97316     — Key components, insights (10%)
Blueprint Navy #1E3A8A       — Flows, connections (5%)
Blueprint Navy #1E3A8A   — Text, labels (5%)
...
SUBTITLE: Same elegant wedge-serif but regular weight (not italic). Smaller size, navy #1E3A8A color, directly below header, sentence case.

LABELS: Geometric sans-serif font (like Avenir or Futura but slightly warmer, clean and technical with even stroke weight). Medium size, navy #1E3A8A color, used for all box labels and component names. Drafted technical versions of this.

INSIGHTS: Condensed italic sans-serif font (sporty editorial style like sports jerseys or magazine callouts, narrow and slanted). Smaller size, Hot Accent Orange #F97316 color, used for callouts with asterisks like "*key insight*".Drafted technical versions of this.

DIAGRAM CONTENT:
Title: '[TITLE]' (Top left, left-justified)
Subtitle: '[SUBTITLE]' (left justified to the Header, slightly below)
Art and labels and such should look like Excalidraw, but drafted technical by a talented Architect Artist that mimics our fonts.
...
All the art components, labels, and such should mostly look drafted technical, similar to Excalidraw. But roughly in the style of our fonts. 
...
```bash
bun Tools/dos-image.ts "[PROMPT]" \
  --intent=diagram \
  --output=/path/to/diagram.png \
  --size=1024x1024 \
  --telemetry-tag=Media/Art/TechnicalDiagrams
```

**Advanced fallthrough — Generate.ts CLI for `--thumbnail` flag:**

```bash
# Blog-header thumbnail variant
...
- [ ] Over-coloring (everything orange/navy)