...
if (process.env.DOS_HOOK_BG !== '1') {
  const _t = startTimer('UpdateTabTitle');
  process.on('exit', () => stopTimer(_t, 'UserPromptSubmit'));
}

runDetachedOrForeground<HookInput>({
  hookName: 'UpdateTabTitle',
  heavyWork,
  foregroundFilter: (data) => {
    const prompt = data.prompt || '';
    if (!prompt || prompt.length < 3) return false;
    if (/^([1-9]|10)$/.test(prompt.trim())) return false;
    return true;
  },
}).catch((err) => {
  console.error(`[UpdateTabTitle] Fatal: ${err}`);
  process.exit(0);
});
