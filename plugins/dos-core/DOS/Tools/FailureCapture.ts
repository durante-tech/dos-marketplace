...
const DOS_DIR = process.env.DOS_DIR || join(process.env.HOME!, '.claude');
...
export async function captureFailure(input: FailureCaptureInput): Promise<string | null> {
  ...
  const failuresDir = join(DOS_DIR, 'MEMORY', 'LEARNING', 'FAILURES', yearMonth);
...
export async function migrateExistingFailures(): Promise<{ migrated: number; errors: string[] }> {
  ...
  const ratingsFile = join(DOS_DIR, 'MEMORY', 'LEARNING', 'SIGNALS', 'ratings.jsonl');
