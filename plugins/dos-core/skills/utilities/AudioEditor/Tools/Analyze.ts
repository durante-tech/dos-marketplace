 ...
 *
 * Routes through Studio gateway when STUDIO_API_URL + STUDIO_API_KEY
 * are set (metered via pooled key). Falls back to BYOK with personal
 * ANTHROPIC_API_KEY from ~/.claude/.env.
 *
...
import { anthropicMessages } from "../../../../Media/src/Lib/anthropic-gateway.ts";
...
// Save
await writeArtifact(outFile, JSON.stringify(merged, null, 2), {
  pack: "Utilities/AudioEditor",
  workflow: "Analyze",
  artifactType: "json",
  title: "audio edit decisions",
});
console.log(`\nSaved: ${outFile}`);
