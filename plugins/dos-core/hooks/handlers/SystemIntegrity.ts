 ...
 *
 * Detects DOS system changes from the transcript and spawns background
...
const STATE_DIR = dosPath('MEMORY', 'STATE');
const STATE_FILE = join(STATE_DIR, 'integrity-state.json');
const INTEGRITY_SCRIPT = dosPath('DOS', 'Tools', 'IntegrityMaintenance.ts');
 ...
 * 2. Filters for DOS system paths (excludes WORK/, LEARNING/)
...
export async function handleSystemIntegrity(
  ...
  console.error(`[SystemIntegrity] ${systemChanges.length} are DOS system changes`);
