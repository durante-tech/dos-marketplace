...
[4] COMPOSITION: Design what to ACTUALLY DRAW ← MANDATORY
     ↓
[5] PROMPT: Construct using DOS Blueprint TECHNIQUE template ← MANDATORY
     ↓
...
**MANDATORY ELEMENTS IN EVERY IMAGE (DOS Blueprint Aesthetic):**
- Engineering blueprint / technical drawing technique (drafted, measured linework)
- Signature (small, monospace, bottom right corner)
- DOS-system-relevant subject matter (ports, hooks, packs, gates, the Algorithm, knowledge graph, write/read paths, gateway, sync tools, MEMORY/, Studio)
- **BLUEPRINT NAVY (#1E3A8A)** — line work, grid, structural elements (MANDATORY)
- **WARM-WHITE PAPER (#FEF7E0)** — substrate / drawing field (MANDATORY; --remove-bg overrides for compositing)
- **HOT ACCENT ORANGE (#F97316)** — the focus element, the hot path, the one thing the diagram is about (MANDATORY)
- --thumbnail flag for blog headers

**🚨 ALL THREE COLORS MUST BE PRESENT IN EVERY IMAGE.**
- Navy: every line, every grid mark, every structural primitive
- Warm-white: the paper / substrate (or transparent for compositing)
- Orange: ONE focal element — the thing the viewer should immediately understand
- The semantic story: "what stands" (navy) → "where the action is" (orange) → "field of operation" (warm-white)
- If an image is missing any of the three, it's INCOMPLETE

**🚨 FORBIDDEN — NEVER INCLUDE:**
- ❌ Borders or frames around the image
- ❌ Decorative backgrounds (grids are functional, not decorative)
- ❌ Burnt sienna #8B4513 (legacy PAI palette — do NOT use)
- ❌ Deep purple #4A148C (legacy PAI palette — do NOT use)
- ❌ Charcoal-sketch / gestural / illustrated rendering (we are drafting, not sketching)
- ❌ Generic AI-stock-art aesthetic (rendered illustrations, soft glows, painterly washes)
- The composition should breathe inside the warm-white field — measured, minimalist
...
**Use deep thinking to construct the final prompt using the DOS Blueprint TECHNIQUE template.**

### Prompt Template

```
Engineering blueprint / technical drawing TECHNIQUE applied to a DOS-system concept.
...
TECHNIQUE — ENGINEERING BLUEPRINT / TECHNICAL DRAWING:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 Drafted measured linework. Not charcoal. Not gestural. 🚨
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- DRAFTED — precise navy ink lines, varying weight by element importance
- MEASURED — dimension callouts, leader arrows, end-ticks, monospace labels
- ORTHOGRAPHIC / ISOMETRIC views — render systems as engineered objects
- LIGHT GRID in background — functional measurement, never decorative
- HATCHING when shading needed — 45° parallel hatching only, never cross-hatching
- NOT charcoal, NOT gestural, NOT illustrated, NOT painted
- Like patent diagrams, NASA technical drawings, SpaceX engineering renderings, Edward Tufte information design, Bell Labs schematics

LINEWORK (applies to ALL subjects):
- Primary structure: heavier navy lines (1.5–2x baseline weight)
- Callouts and dimensions: thinner navy lines, monospace labels
- Hot-path / focal element: outlined or highlighted in HOT ORANGE (#F97316)
- All lines confident, drafted, geometric — never sketchy
- UNIFIED technical-drawing quality across all elements

DOS PRIMITIVES (if present) — RENDER AS ENGINEERED OBJECTS:
- Ports / hooks / packs / gates: drawn as labelled mechanical components
- Data streams / signals / hot paths: directional arrows in HOT ORANGE
- Knowledge graph / MEMORY/ subdirs: rendered as nodes with edges, dimension callouts
- Studio gateway / sync tools: rendered as cross-section pipes or chambers
- The Algorithm phases: rendered as a numbered sequence with state-transition arrows
- Each major element labelled with leader arrow + monospace caption (e.g. `PORT 1`, `HOOK :: PostToolUse`, `RATIO 20→1`)

HUMAN FIGURE (if present) — RENDERED AS DRAFTED FIGURE:
- The operator/human appears as a small drafted figure in navy line
- Drawn with same engineering-drawing quality as the system primitives
- Often a "user" silhouette per ISO/ANSI convention — clear, simple, scaled
- Figure may interact with the system (hand on a port, finger on a button) — rendered with measured lines
- The figure is NEVER the focal point unless the post is about the operator — system is primary

OBJECTS (if present) — RENDERED AS ENGINEERED FORMS:
- Mechanical objects: orthographic projection with hidden lines (dashed)
- Data objects (records, JSON, files): rendered as labelled tablets / cards
- Connections: clear leader arrows, no ambiguity
- Recognizable forms through drafted lines, not gesture

COMPOSITION — FULL FRAME, MEASURED LAYOUT:
- 🚨 SUBJECTS FILL THE FRAME — edge to edge, balanced
- Light navy grid in background suggests measurement
- Leave breathing room around the focal HOT ORANGE element so it pops
- Margins exist for callouts and labels — not for emptiness
- MINIMALIST means few elements, each measured and labelled

COLOR — NAVY-DOMINANT WITH ORANGE FOCAL ACCENT:
- BLUEPRINT NAVY (#1E3A8A) — 65–75% of image (lines, grid, structure)
- WARM-WHITE PAPER (#FEF7E0) — substrate / 20–30% visible
- HOT ACCENT ORANGE (#F97316) — exactly ONE focal area / 5–10% of image
- The semantic story: navy structure → orange focus → warm-white field
- Every line and label belongs to a form — no decorative marks

Sign small in monospace cursive in BOTTOM RIGHT CORNER.
NO other text in the image except element labels and dimension callouts.
```
...
- [ ] "Engineering blueprint", "drafted", "callouts" explicitly stated
- [ ] Reference appropriate to subject (NASA / Tufte / patent / Bell Labs / SpaceX)
...
|-----------|------|-------------|
| "fast", "quick", "draft" | `--model flux` | Fast iteration, reliable |
| (default), "best", "high quality" | `--model nano-banana-pro` | Best quality + refs (Gemini) |
| "cinematic", "film-like" | `--model seedream` | Cinematic film aesthetics |
| "highest quality", "max" | `--model flux-2-max` | Maximum Flux quality |
| "detail", "typography" | `--model imagen-4-ultra` | Fine detail + text rendering |
...
bun ~/.claude/DOS/Tools/RemoveBg.ts /path/to/output.png

# Or batch process multiple images
bun ~/.claude/DOS/Tools/RemoveBg.ts image1.png image2.png image3.png
```
...
|-------|---------|-------------|
| **seedream** | `--model seedream --size 1:1 --remove-bg` | Cinematic film-like quality |
| **flux-2-max** | `--model flux-2-max --size 1:1 --remove-bg` | Highest Flux quality |
| **imagen-4-ultra** | `--model imagen-4-ultra --size 1:1 --remove-bg` | Fine detail + typography |
...
- [ ] **SIGNATURE PRESENT** — signed small in monospace, bottom right corner (if requested)
- [ ] **PROBLEM TYPE VISIBLE** — the problem type (sorting, double standard, asymmetry, blindspot, etc.) is immediately obvious
- [ ] **Subject is a DOS primitive or system concept** — ports, hooks, packs, gates, the Algorithm, knowledge graph, write/read paths, gateway, sync tools, MEMORY/, Studio
- [ ] **Concrete elements visible** — key nouns/metaphors from content actually appear as drafted objects
- [ ] **Title test passes** — someone could guess the topic from the image alone
- [ ] **Labels readable** — element labels and dimension callouts are clearly visible and correctly rendered
- [ ] **NOT defaulting to literal buildings** — DOS primitives rendered as engineered objects, not architecture
...
- [ ] **Background removed** — transparent background (or warm-white substrate when not compositing)

**TECHNIQUE (all required):**
- [ ] Engineering-blueprint quality — drafted, measured, intentional linework
- [ ] NOT charcoal, NOT gestural, NOT illustrated, NOT painted
- [ ] Confident geometric lines — varying weight by element importance
- [ ] Dimension callouts and leader arrows present where useful
- [ ] Looks like a patent diagram or NASA technical drawing

**FIGURE STYLE (if figures present):**
- [ ] **DRAFTED FIGURE** — clear, simple, scaled per ISO/ANSI convention
- [ ] **Same drawing quality as the system primitives** — not separately rendered
- [ ] **Functional, not emotional** — the figure interacts with the system; expression is implied via posture, not face detail
- [ ] **Navy line + occasional orange accent** for what they're touching/operating
- [ ] **Looks like a technical-manual user illustration** — not a charcoal sketch

**COLOR (all required — ALL THREE COLORS MANDATORY):**
- [ ] **BLUEPRINT NAVY (#1E3A8A)** — 65–75% of image (lines, grid, structure)
- [ ] **WARM-WHITE PAPER (#FEF7E0)** — substrate / 20–30% visible (or transparent when --remove-bg used)
- [ ] **HOT ACCENT ORANGE (#F97316)** — exactly ONE focal element / 5–10% of image
- [ ] No legacy `#8B4513` sienna or `#4A148C` purple anywhere
- [ ] Orange placement makes the focal point obvious at thumbnail size

**EMOTION (all required):**
- [ ] Emotional register clear — matches Step 2 selection
- [ ] Layout reinforces the meaning (asymmetry shown asymmetric, balance shown balanced)
- [ ] Focal-orange placement serves the narrative
- [ ] Overall composition argues the same point as the content

**COMPOSITION (all required):**
- [ ] **FULL FRAME** — subjects fill the field, balanced layout
- [ ] **MEASURED** — light grid suggests the diagram was engineered
- [ ] **FOCAL ORANGE POPS** — the one orange element is unmistakable
- [ ] **MONOSPACE SIGNATURE** — small cursive in BOTTOM RIGHT CORNER
- [ ] **CALLOUTS PRESENT** — leader arrows + monospace labels where useful

**QUALITY (all required):**
- [ ] Could appear in a NASA technical-document appendix
- [ ] Could be a figure in an Edward Tufte information-design book
- [ ] Distinctive — NOT generic AI illustration, NOT corporate-flat-design clip-art
- [ ] Sophisticated — rewards closer looking; callouts have meaning
- [ ] **Transparent background** — used `--remove-bg` flag (or warm-white substrate retained if compositing on dark)
...
| Problem | Fix |
|---------|-----|
| **Subjects too small** | 🚨 Add "LARGE PRIMITIVES that FILL THE FRAME", "balanced measured layout, no empty corners" |
| **Too much empty space** | 🚨 Add "callouts and dimension labels populate margins", "field is measured not empty" |
| **Background dominates** | 🚨 Add "primitives are dominant focus", "navy linework defines structure" |
| **Concept not recognizable** | Add "DOS PRIMITIVE: [port / hook / pack / gate]" with "labelled callouts make the system explicit" |
| **Looks too sketchy / gestural** | 🚨 Add "DRAFTED, MEASURED LINES", "patent-diagram quality", "no hatching except 45° parallel where shading needed" |
| **Lines are wobbly / hand-drawn-feeling** | 🚨 Add "ruler-drafted geometric lines", "engineering-drawing precision", "varying weight by importance" |
| **Primitives look flat / abstract** | 🚨 Add "orthographic / isometric views", "dimension callouts", "hidden lines dashed" |
| **No focal element** | Add "ONE orange-highlighted focal element", "the one thing the diagram is about pops in #F97316" |
| **Too illustrated / rendered** | Add "TECHNICAL DRAWING quality", "drafted not painted", "like NASA / SpaceX engineering renderings" |
| **Labels missing or unclear** | Add "monospace labels with leader arrows", "PORT 1, HOOK :: PostToolUse style naming" |
| **Wrong emotion** | Adjust LAYOUT — asymmetric for asymmetry, balanced for balance, sealed walls for blocked paths |
| **Colors wrong / muddy** | Emphasize "BLUEPRINT NAVY #1E3A8A linework on WARM-WHITE #FEF7E0 paper, ONE focal element in HOT ORANGE #F97316" |
| **Generic AI look** | Add "patent diagram", "Tufte information design", "Bell Labs schematic", "ISO/ANSI engineering drawing" references |
...
3. EMOTION → Match to register in ~/.claude/DOS/aesthetic.md
4. COMPOSITION → Design what to DRAW (content-relevant, NOT defaulting to architecture)
5. PROMPT → Build using DOS Blueprint TECHNIQUE template
...
| Content About... | Register | Layout Treatment | Orange Placement |
|------------------|----------|------------------|------------------|
| Architectural flaw / asymmetry | Discovery / Revelation | Asymmetric layout — what's broken is visible | Orange marks the broken/missing element |
| New capability shipped | Determination | Solid grounded layout, primitive fully realized | Orange marks the "now-active" path |
| Strategic decision | Contemplation | Balanced, options laid out as parallel diagrams | Orange marks the chosen path |
| Security / risk | Urgency | Fractured layout, breach visible | Orange marks the breach point |
| Discovery / breakthrough | Wonder | Revelatory layout, the new primitive emerges | Orange marks what just came online |
| Refactor / cleanup | Effort | Before/after diagrams, clear improvement | Orange marks the cleaned-up structure |
| Deprecation / removal | Melancholy | Fading or struck-through primitive | Orange marks the surviving path |
| System integration | Connection | Multi-primitive diagram with clear edges | Orange marks the new connection |

### The Brand Look Checklist

Before submitting any image:
- ✅ **Subject is a DOS primitive** — ports, hooks, packs, gates, the Algorithm, knowledge graph, write/read paths, gateway, sync tools, MEMORY/, Studio
- ✅ **CSE-24 was run** — actually executed the story explanation command
- ✅ **Concrete elements visible** — key nouns/metaphors from content appear as drafted objects
- ✅ Engineering-blueprint TECHNIQUE — drafted, measured, callouts
- ✅ Register clear and intentional
- ✅ Three-color palette enforced — navy lines + warm-white substrate + ONE orange focal
- ✅ Could appear in a NASA appendix or Tufte book — not generic AI
- ✅ **--thumbnail flag used** — both transparent and warm-white versions generated
- ✅ **OPTIMIZATION COMPLETED** — resized to 1024, converted to WebP, optimized thumbnails created
- ✅ Signature — small monospace cursive bottom right (optional)

---

**The workflow: UNDERSTAND → CSE-24 → REGISTER → COMPOSITION → PROMPT (DOS Blueprint) → GENERATE (--thumbnail) → OPTIMIZE → VALIDATE → Complete**