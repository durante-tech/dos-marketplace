---
name: Batch Entity Extraction Gemini3
description: Batch-extract structured entities from content using Gemini 3 Pro.
status: STABLE
---
...
**Option B: Scrape.fetch standard (Better Quality)**
```typescript
import { Scrape } from "Packs/scraping/src"

const contents = await Promise.all(
  urls.map(async (url) => {
    const { markdown } = await Scrape.fetch(url)
    return { url, markdown }
  })
)
// Default (standard) mode — router escalates WebFetch → proxy tier for anti-bot pages (RFC-0015 §7.1)
```

**Option C: Scrape.fetch deep (Production — anti-bot + semantic cleanup)**
```typescript
import { Scrape } from "Packs/scraping/src"

const contents = await Promise.all(
  urls.map(async (url) => {
    const { markdown } = await Scrape.fetch(url, { mode: "deep" })
    return { url, markdown }
  })
)
// Full ladder: WebFetch → proxy tier → semantic cleanup (RFC-0015 §5.1, §7.1)
```
...
# FIRST: materialize utils/process-batch.ts from the inline sample below — it does NOT ship with the skill
