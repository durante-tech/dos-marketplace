...
# Check for DOS MEMORY structure (optional, enhances results)
if [ -d "$CLAUDE_DIR/MEMORY/WORK" ]; then
  echo "OK DOS MEMORY/WORK directory exists (full functionality available)"
else
  echo "INFO DOS MEMORY/WORK not found (command will work but may return fewer results)"
fi
...
- DOS MEMORY structure: [found (full functionality) / not found (basic functionality)]
- Git repository: [found / not found]

[If MEMORY not found]: Note: Context Search searches DOS's MEMORY directories
for prior work sessions. Without DOS installed, the command will still work but
will only search git history. For full functionality, consider installing DOS:
https://github.com/durante-tech/dos"
```
...
[ -f "$CLAUDE_DIR/MEMORY/STATE/work.json" ] && echo "  OK work.json — session registry available" || echo "  INFO work.json — not found (install DOS for this feature)"
[ -d "$CLAUDE_DIR/MEMORY/WORK" ] && echo "  OK MEMORY/WORK — PRD directory available" || echo "  INFO MEMORY/WORK — not found (install DOS for this feature)"
[ -f "$CLAUDE_DIR/MEMORY/STATE/session-names.json" ] && echo "  OK session-names.json — session names available" || echo "  INFO session-names.json — not found (install DOS for this feature)"
...
Note: Results improve the more you use DOS's work tracking system (PRDs, session names, git commits)."
```

### On Success (Without DOS MEMORY)

```
"Context Search v1.1.0 installed successfully!

The commands are ready, but DOS's MEMORY system isn't installed yet.
Right now, the commands will search git history only.

For full functionality (PRD search, session registry, work directories), install DOS:
https://github.com/durante-tech/dos
...
This is expected if DOS's MEMORY system isn't installed. The command searches DOS-specific directories. Options:
1. Install DOS for full work tracking: https://github.com/durante-tech/dos
2. The command will still search git history if `~/.claude/` is a git repo

### Command works but results are sparse

Context Search searches data created by DOS's Algorithm (PRDs, session metadata). The more you use DOS's structured workflow, the richer the search results become.
...
If you have DOS installed, Context Search becomes significantly more powerful:
...
Without DOS, the command still works but only searches git history.
