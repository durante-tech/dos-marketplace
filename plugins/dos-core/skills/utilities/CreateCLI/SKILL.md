...
name: CreateCLI
description: Generate complete TypeScript CLIs with 3-tier template system (simple argv, yargs, oclif), type safety, error handling, and documentation. USE WHEN create CLI, build CLI, command-line tool, wrap API, add command, upgrade tier, TypeScript CLI. NOT for in-repo DOS Tools/ utilities (use bun Tools/scaffold-tool.ts — house style is hand-rolled argv, no CLI frameworks; standard at Docs/TOOLING.md).
role: executor
accepts:
  - text
roots:
...
`~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/CreateCLI/`
...
Generate complete TypeScript CLIs with comprehensive documentation, type safety, error handling, and CLI-First Architecture principles.

> **Fence — in-repo DOS `Tools/` utilities are NOT this skill's domain.** CreateCLI builds standalone CLI *products* (own package.json, framework tiers). A utility living in `~/Durante/Tools/` follows the house standard instead — hand-rolled `process.argv`, zero CLI frameworks (168/168 tools) — and is scaffolded with `bun Tools/scaffold-tool.ts <kebab-name> --desc "…"`. Rationale: `Docs/TOOLING.md`.
...
## 🏗️ INTEGRATION WITH DOS

### Technology Stack Alignment

Generated CLIs follow DOS standards:
...
- `${PROJECTS_DIR}/DOS/Examples/clis/` - Example CLIs (PUBLIC repo)
...
ghcli issues create --repo dos --title "Bug fix"
...
### 5. Integration
- ✅ Follows DOS tech stack (Bun, TypeScript)
- ✅ CLI-First Architecture principles
- ✅ Deterministic output (JSON)
...
3. **Complete, Not Scaffold** - Every CLI ships complete
...
**This skill turns "I need a CLI for X" into complete tools in minutes, following proven patterns from llcli and CLI-First Architecture.**
