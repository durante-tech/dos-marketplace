...
```
Read $DOS_DIR/MEMORY/RESEARCH/WorldModels/INDEX.md (if exists)
...
```bash
bash "$DOS_DIR/DOS/Tools/voice.sh" custom --voice-id fTtv3eikoepIosk8dTZ5 "Updating world threat models. This will take several minutes as I research current state for each time horizon."
```
...
For each model being created or updated, **spawn a parallel Research subagent** (one Task per horizon). Threat-horizon modeling is the canonical use case for `ExtensiveResearch` — multi-perspective, citation-backed, deep mode is appropriate because models drive testable predictions and high-stakes decisions.

**Spawn pattern (one Task call per horizon being modeled — fan out in a single parallel batch):**

```ts
// Example — near-term batch (6mo, 1yr, 2yr, 3yr → 4 parallel spawns)
Task({
  subagent_type: "general-purpose",
  description: "World model research — 6mo horizon",
  prompt: "Invoke the research skill, ExtensiveResearch workflow. Query: 'Current global state and projections for the 6-month timeframe across geopolitics, AI and technology trajectory, economics, society, environment, and security. Focus on developments since {LAST_UPDATED or baseline}. Surface current events, immediate trends, and named forecasts with citations.' Return: structured findings per the ExtensiveResearch output shape — multi-source synthesis with verbatim citations, named entities, specific numbers, and confidence ratings. Under 1,500 words."
})

Task({
  subagent_type: "general-purpose",
  description: "World model research — 5yr horizon",
  prompt: "Invoke the research skill, ExtensiveResearch workflow. Query: 'Current global state and projections for the 5-year timeframe across geopolitics, AI and technology trajectory, economics, society, environment, security. Focus on trend trajectories, structural shifts, and emerging patterns since {LAST_UPDATED or baseline}.' Return: structured findings per ExtensiveResearch — multi-source synthesis with citations, named entities, specific numbers, confidence ratings. Under 1,500 words."
})

// Continue one Task per horizon being modeled (near-term: 6mo/1yr/2yr/3yr; mid-term: 5yr/7yr/10yr; long-term: 15yr/20yr/30yr/50yr).
```

**Per-horizon query calibration:**
- **Near-term (6mo-3yr):** "current events, immediate trends, specific forecasts"
- **Mid-term (5yr-10yr):** "trend trajectories, structural shifts, emerging patterns"
- **Long-term (15yr-50yr):** "megatrends, demographic forces, technological paradigm shifts"

**Batching rule:** Fan out all horizons in scope as parallel Task spawns in a single message. Do NOT serialize — the Research subagent handles its own internal fan-out per `ExtensiveResearch`'s multi-agent pattern.

**If user provided analysis:** Integrate user's insights as primary source in the synthesis step (Step 4); the Research spawns still run as the supporting evidence layer.
...
Save to: `$DOS_DIR/MEMORY/RESEARCH/WorldModels/{horizon}.md`

### Step 5: Update INDEX

Write/update `$DOS_DIR/MEMORY/RESEARCH/WorldModels/INDEX.md`:
...
```bash
bash "$DOS_DIR/DOS/Tools/voice.sh" custom --voice-id fTtv3eikoepIosk8dTZ5 "World models updated. N horizons refreshed with current research."
```
