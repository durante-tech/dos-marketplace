...
`~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/Prompting/`
...
- Uses `RenderTemplate.ts` for eval prompt generation
...
The templating system eliminated **~35,000 tokens (65% reduction)** across DOS:
...
**Philosophy:** Prompts that write prompts. Structure is code, content is data. Meta-prompting enables dynamic composition where the same template with different data generates specialized agents, workflows, and evaluation frameworks. This is core DOS DNA - programmatic prompt generation at scale.
