...
- World models must exist at `$DOS_DIR/MEMORY/RESEARCH/WorldModels/`
...
```
Check $DOS_DIR/MEMORY/RESEARCH/WorldModels/ for all 11 model files.
...
```bash
bash "$DOS_DIR/DOS/Tools/voice.sh" custom --voice-id fTtv3eikoepIosk8dTZ5 "Testing your idea against all eleven world threat models at TIER tier"
```
...
Read all 11 model files from `$DOS_DIR/MEMORY/RESEARCH/WorldModels/`.
...
```bash
bash "$DOS_DIR/DOS/Tools/voice.sh" custom --voice-id fTtv3eikoepIosk8dTZ5 "Analysis complete. SUMMARY_OF_EXECUTIVE_VERDICT"
```
