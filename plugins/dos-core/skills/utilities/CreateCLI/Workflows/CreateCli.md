---
name: Create Cli
description: Generate a production-quality TypeScript CLI following the CLI-First Architecture.
status: STABLE
---
...
function loadConfig(): Config {
  const envPath = process.env.DOS_CONFIG_DIR ? join(process.env.DOS_CONFIG_DIR, '.env') : join(homedir(), '.config', 'DOS', '.env');
  ...
  try {
    ...
    if (!apiKey) {
      console.error('Error: {{ENV_VAR_NAME}} not found in ${DOS_CONFIG_DIR}/.env');
      process.exit(1);
    }
    ...
    console.error(`Error: Cannot read ${DOS_CONFIG_DIR}/.env file`);
    console.error('Make sure {{ENV_VAR_NAME}} is set in ${DOS_CONFIG_DIR}/.env');
    process.exit(1);
  }
}
```
...
**Reference:** `~/.claude/DOS/CliFirstArchitecture.md` (Configuration Flags section)
...
Next steps:
1. Configure: Add {{ENV_VAR_NAME}} to ${DOS_CONFIG_DIR}/.env
...
Next steps:
1. Add NOTION_API_KEY=your_key to ${DOS_CONFIG_DIR}/.env
