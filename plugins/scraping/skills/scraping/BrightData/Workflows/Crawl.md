...
**Purpose:** Crawl multiple pages from a website — following links, extracting content, and returning structured results. Two modes: **Light Crawl** (agent-driven, using code-first batch scraping) and **Full Crawl** (Bright Data Crawl API for entire sites).
...
|------|-------|--------|------|----------|
| **Light Crawl** | 1-50 pages | Code-first `batchScrape()` + link extraction loop | ~$0.006/page | Section of a site, specific content |
...
Uses code-first `batchScrape()` from gateway shim (up to 10 URLs per call) with iterative link discovery.
...
Use the code-first BrightData client to fetch pages in batches with bounded concurrency.

```bash
# Via CLI tool:
bun ~/.claude/skills/scraping/BrightData/batch-scrape.ts [urls...]

# Or via code:
import { batchScrape } from "../../Lib/brightdata-gateway.ts";
const { results } = await batchScrape({ urls, dataFormat: "markdown" });
```

```
For each batch of unvisited URLs (5 concurrent):
  1. Call batchScrape with the URLs
...
const snapshotId = await client.startCrawl("[STARTING_URL]", {
  depth: 3,
  urlFilter: "[REGEX_PATTERN]",
  format: "json",
  datasetId: process.env.BRIGHTDATA_CRAWL_DATASET_ID,
});
```

Or via Studio gateway (metered):
```bash
curl -X POST "${STUDIO_API_URL}/api/v1/scraping/brightdata/crawl" \
  -H "Authorization: Bearer ${STUDIO_API_KEY}" \
  -H "Content-Type: application/json" \
  -d '{"url": "[STARTING_URL]", "depth": 3, "urlFilter": "[REGEX_PATTERN]"}'
```
...
```typescript
const status = await client.getCrawlStatus(snapshotId);
// status.status: 'running' | 'ready' | 'failed'
```
...
```typescript
const pages = await client.getCrawlResults(snapshotId);
// pages: Array<{ url: string; content: string }>
// Content is pre-filtered — only URL + markdown/text returned
```
...
- API auth failure → check `BRIGHTDATA_API_TOKEN` environment variable
