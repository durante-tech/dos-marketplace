...
> Canonical identity schema for all DOS agents — built-in and custom.
...
\`\`\`bash
bash "$DOS_DIR/DOS/Tools/voice.sh" custom \
  --voice-id {voiceId} \
  --voice-settings '{"stability":{voice.stability},"similarity_boost":{voice.similarity_boost},"style":{voice.style},"speed":{voice.speed},"use_speaker_boost":{voice.use_speaker_boost}}' \
  "[Agent name] activated, loading context"
\`\`\`
...
Every response must include a voice notification call:
\`\`\`bash
bash "$DOS_DIR/DOS/Tools/voice.sh" custom \
  --voice-id {voiceId} \
  --voice-settings '{"stability":{voice.stability},"similarity_boost":{voice.similarity_boost},"style":{voice.style},"speed":{voice.speed},"use_speaker_boost":{voice.use_speaker_boost}}' \
...
[Standard DOS output format with sections:
...
| Lifecycle | Permanent, ships with DOS | User-managed (create/delete) |
