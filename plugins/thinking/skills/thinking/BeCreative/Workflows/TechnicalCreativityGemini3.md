---
name: Technical Creativity Gemini3
description: 
status: STABLE
---
...
> **Model pin (de-versioned).** Use the durable family name `gemini-3-pro`, not a dated/preview tag like `gemini-3-pro-preview`. A family name survives the provider's point releases; a dated preview tag rots — and hard-breaks for a customer without preview access — the moment Google ships the next snapshot or retires the preview. Mirrors the Research-pack convention in `Packs/research/src/Tools/model-pins.ts` ("prefer durable family names over dated release tags").
>
> **Graceful degradation (no hard break).** If `gemini-3-pro` is unavailable to your account (no access, deprecated, or the CLI errors with unknown-model / 404), fall back to a stable Gemini model — `gemini-2.5-pro` (the `GEMINI_MODELS` family pinned in `model-pins.ts`). Recommended invocation: `MODEL="${LLM_GEMINI_MODEL:-gemini-3-pro}"; llm -m "$MODEL" "..."` and re-run with `MODEL=gemini-2.5-pro` on failure. Never abort the workflow over a single model id — the examples below use `gemini-3-pro` literally for readability; substitute `$MODEL` to make them degradation-safe.

```bash
llm -m gemini-3-pro "Generate 5-10 diverse creative technical solutions for this problem:
...
```bash
llm -m gemini-3-pro "Generate 5 diverse creative caching strategies for this problem:
...
```bash
llm -m gemini-3-pro "Generate 5-10 creative sorting/selection algorithms for this streaming top-K problem:
...
```bash
llm -m gemini-3-pro "Generate 5 creative database architectures for this IoT time-series problem:
...
```bash
llm -m gemini-3-pro "Generate 5-10 creative rate limiting algorithms beyond traditional token bucket:
...
llm -m gemini-3-pro "Generate 5 high-level architectural approaches for [problem]..."

# Stage 2: For selected approach, generate detailed component designs
llm -m gemini-3-pro "For this architecture: [selected approach]
...
llm -m gemini-3-pro "For this component: [selected component]
...
```bash
llm -m gemini-3-pro "Generate solutions for [technical problem] by applying patterns from:
...
```bash
llm -m gemini-3-pro "Generate solutions for [problem] under different constraint scenarios:
...
```bash
llm -m gemini-3-pro "Generate 5-10 diverse creative technical solutions for:
