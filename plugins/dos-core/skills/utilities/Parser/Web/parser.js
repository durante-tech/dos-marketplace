...
class ParserApp {
    ...
    async handleParse() {
        const { valid: urls, rejected } = this.getURLs();

        if (urls.length === 0 && rejected.length === 0) {
        ...
        if (urls.length === 0) {
            this.showError(
                'No valid URLs found. Skipped invalid lines:\n' + rejected.join('\n')
            );
            return;
        }
        ...
        if (rejected.length > 0) {
            this.showError(
                `Skipping ${rejected.length} invalid line(s):\n` + rejected.join('\n')
            );
        }
    ...
    createMockResult(url, contentType) {
        ...
        return {
            ...
            extraction_metadata: {
                ...
                warnings: ["DEMO MODE: this record is synthetic mock data, not a real parse. No network fetch occurred."],
                demo_mode: true,
                version: "1.0.0"
            }
        };
    ...
    showResults() {
        ...
        this.failedCount.textContent = `✗ ${this.failedNum} failed`;
    ...
    createResultItem(result, index) {
        ...
        const isDemo = result.extraction_metadata && result.extraction_metadata.demo_mode === true;
        const demoBadge = isDemo
            ? '<span class="demo-badge" title="Synthetic mock data — not a real parse">DEMO / MOCK</span>'
            : '';

        item.innerHTML = `
            <div class="result-header" data-index="${index}">
                <div class="result-title">
                    <h3>${this.escapeHtml(result.content.title)} ${demoBadge}</h3>
                    <p>${filename}</p>
                </div>
