---
name: Scraping
pack-id: durante-scraping-v1.0.0
version: 1.0.0
author: durante-tech
...
4. **Tier 4: Bright Data code-first (SmartFetch + gateway)** -- Professional proxy network with CAPTCHA solving and residential IPs
...
- **Dependencies:** bun (for Apify TypeScript tools), Apify API token (for social media scraping), Bright Data code-first client (for Tier 4 proxy scraping)
...
**Tier 4 requires:** Bright Data code-first client configured. The client provides `BrightData.scrapeAsMarkdown()` and `BrightData.batchScrape()` methods.
...
- Set as `APIFY_TOKEN=apify_api_xxxxx...` in your environment or DOS .env
...
- **DOS Research Skill** -- Research routes to Scraping for content retrieval and web scraping workflows
...
- **DOS OSINT Skill** -- OSINT investigations leverage Scraping for social media and business intelligence
