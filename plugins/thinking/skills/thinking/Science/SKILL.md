---
name: Science
description: Hypothesis-test-analyze cycles for explicit experiment design and cross-domain coordination — the methodological substrate the other Thinking modes embody (see Protocol.md), invoked directly only for method-specific work, never as a catch-all router peer. Includes define goal, generate hypotheses, design experiment, measure results, analyze results, iterate, full cycle, quick diagnosis, and structured investigation. USE WHEN hypothesis, experiment, define goal, design experiment, measure results, analyze results, generate hypotheses, iterate on results, full cycle, quick diagnosis, structured investigation, science.
role: thinker
accepts:
  - text
roots:
...
`~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/Science/`

If this directory exists, load and apply any PREFERENCES.md, configurations, or resources found there. These override default behavior. If the directory does not exist, proceed with skill defaults.

# Science - The Universal Algorithm

**The scientific method applied to everything — the methodological substrate the other six Thinking modes specialize. Skills don't call Science; they implement it (see `Protocol.md`). Invoke it directly only for explicit experiment design or cross-domain coordination, never as a catch-all router peer.**

> **Self-differentiation:** Unlike the other six modes (each a concrete specialization you invoke for a task), Science is the cycle they all embody. Its architectural value is lineage, not routing greed — vague "think about / figure out" intent is NOT Science's to claim; it routes to the parent's disambiguation list. The Protocol stays a protocol skills implement, never a service they call (see `Protocol.md`, "Service (Wrong)").
...
## Compose-with (advisory)

*Suggested hand-offs, not executable ownership — Science does not runtime-invoke these skills; the mapping is advisory pedagogy (see `Protocol.md`).*

| Phase | Skills to compose with |
|-------|-----------------|
