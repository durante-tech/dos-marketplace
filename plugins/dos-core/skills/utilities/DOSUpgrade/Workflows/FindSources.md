---
name: Find Sources
description: Discover and evaluate new sources to add to upgrade monitoring.
status: STABLE
---
...
- Content relevance to DOS infrastructure
...
**URL:** [url]
**Relevance:** [Why this matters for DOS]
...
Edit `~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/DOSUpgrade/youtube-channels.json`

### For Other Sources:
Currently, non-YouTube sources are monitored via the base `sources.json`.
To request additions to base Anthropic monitoring, note them for next DOS release.
```
...
cat ~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/DOSUpgrade/youtube-channels.json
