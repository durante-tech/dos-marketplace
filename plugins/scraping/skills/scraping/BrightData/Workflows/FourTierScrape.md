...
- Bright Data code-first client available
...
### Step 4: Tier 4 - Bright Data Code-First (Professional Scraping)

**Description:** Use Bright Data's Web Unlocker API via code-first client for professional scraping with bot detection bypass

**Actions:**
```bash
# Run the code-first scraper via the RFC-0031 CLI Port (from repo root)
bun Tools/dos-scrape.ts "[URL]"
```

(The underlying scraper module is `BrightData/Tools/SmartFetch.ts` — a library, not a CLI; consume it via the CLI Port above or the gateway shim below, never `bun …SmartFetch.ts` directly.)

Or via the gateway shim (Studio-aware):
```typescript
import { scrapeUrl } from "../Lib/brightdata-gateway.ts";
...
- **98% token savings** vs MCP (filtered in code before returning)
...
Success? → Yes → Return content ✓
  ...
  No
  ↓
Attempt Tier 4 (Bright Data code-first client)
  ↓
Success? → Yes → Return content ✓
  ↓
  No
...
Output:
```markdown
Successfully retrieved content from https://protected-site.com using Tier 4 (Bright Data code-first client)
...
Output:
```markdown
Retrieved content from https://any-site.com using Tier 4 (Bright Data code-first client) as requested
