...
/**
 * Load environment variables from ${DOS_DIR}/.env
...
async function main() {
  try {
    ...
    if (!botToken) {
      throw new CLIError(
        'Missing DISCORD_BOT_TOKEN environment variable. Add it to ${DOS_DIR}/.env'
      );
    }

    if (!channelId) {
      throw new CLIError(
        'Missing MIDJOURNEY_CHANNEL_ID environment variable. Add it to ${DOS_DIR}/.env'
      );
    }
