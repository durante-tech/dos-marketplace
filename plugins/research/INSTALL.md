...
elif [ -f "$HOME/Projects/DOS/.env" ] && grep -q "PERPLEXITY" "$HOME/Projects/DOS/.env" 2>/dev/null; then
  echo "OK Perplexity API key found in DOS .env"
else
...
# Check for DOS MEMORY structure
if [ -d "$CLAUDE_DIR/MEMORY" ]; then
  echo "OK DOS MEMORY directory exists (research artifacts will be stored)"
else
  echo "INFO DOS MEMORY not found (research output will use working directory)"
fi

# Check for user customizations
if [ -d "$CLAUDE_DIR/DOS/USER/SKILLCUSTOMIZATIONS/Research" ]; then
...
Check that your Perplexity API key is valid and has credits. The key should be in your DOS .env file or exported as `PERPLEXITY_API_KEY`.
...
Ensure `~/.claude/MEMORY/RESEARCH/` directory exists. If DOS MEMORY is not installed, create it manually: `mkdir -p ~/.claude/MEMORY/RESEARCH/`
