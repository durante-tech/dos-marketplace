...
- **v5.1.0** (2026-04-26) — Per-step durability via `CheckpointPerISC` hook (PostToolUse on PRD edits, auto-commits per ISC transition with `--no-verify --no-gpg-sign`, sidecar idempotency, allowlist for opt-in repos). Doctrine elsewhere unchanged from v5.0.0.
...
- **v4.1.0** (2026-04-23) — Vocabulary release: PRD → PRD (Ideal State Artifact). No doctrine changes.
- **v4.0.1** (2026-04-22) — Patch: `[A]` category parser fix in `prd-utils.ts`, reflection JSONL gains `prd_level_audit` and `hva` fields.
- **v4.0.0** (2026-04-21) — Frame-shift: PRD-as-unit (Coverage / Tightness / Uniqueness — the PRD-Level HVA). Doctrine preamble added.
...
- To **roll back** to a prior version: see the migration recipe in that version's section below. Hooks parse only the PRD frontmatter `effort` enum and ISC checkboxes — both version-stable — so rollback is non-destructive. (v5.3.0 also dropped category tags from the parse contract; legacy bracketed PRDs are still accepted via backward-compat parsing.)
...
v6.2.0 ships the long-running PRD expansion conversation Lucas and Durante had across the 2026-04-27/28 turn boundary. The doctrine bump preserves v6.1.0's thinking-floor hardening verbatim; everything new is additive.

**The work captured in C/R/L form (eat-our-own-dog-food on the new Changelog format):**

- 2026-04-28 | conjectured: the PRD's five sections (Goal/Context/Criteria/Decisions/Verification) carry enough structure for any project's articulation of ideal state
  ...
  learned: the PRD body needs twelve sections in fixed order, with hard tier-completeness gating that prevents ceremony bloat at E1 (just Goal+Criteria) while requiring full structure at E4/E5 (all twelve, plus active Interview at E5)
  criterion now: v6.2.0.md PRD-as-System-of-Record subsection lists the twelve sections, the PRDFormat.md spec bumps to v2.7 with the full schema, and the Tier Completeness Gate is HARD at every tier

- 2026-04-28 | conjectured: the existing skill-and-hook surface was sufficient infrastructure for the PRD primitive
  refuted by: filesystem check showed no prd skill on disk, PRDFormat.md was the only documentation, no project PRDs existed anywhere, and the changelog format that v6.2.0 introduces (conjecture/refutation/learning) had no canonical writer
  ...
  criterion now: new `~/.claude/skills/prd/` skill with Scaffold / Interview / CheckCompleteness / Reconcile / Seed / Append workflows, and Append specifically gates the C/R/L format (refuses partial entries)
...
- 2026-04-28 | conjectured: Out of Scope and Anti-criteria are the same concept, just at different granularities — both can fold into anti-criteria ISCs
  refuted by: Lucas articulated the distinction directly — Out of Scope is declarative ("we are not building mobile native apps"), anti-criteria are derived testable probes ("`/admin` returns 404")
  learned: the three-guardrail taxonomy needs four surfaces, not three: Principles bind thinking, Constraints bind solution space, Out of Scope binds the vision (anti-vision), Anti-criteria bind the test surface (derived). Out of Scope is its own PRD section, not an ISC type
  criterion now: PRD body has `## Out of Scope` as section 3, distinct from `## Criteria` anti-criteria

- 2026-04-28 | conjectured: ephemeral feature files (Ralph Loop / Maestro pattern) need a separate workflow file in the prd skill
  refuted by: Scaffold workflow already had to know how to build the master PRD from a prompt; the ephemeral excerpt is the same operation against a feature-scoped subset
...
- Advisor flagged two real risks during the v6.2.0.md write: (1) E1 fast-path bypass risks becoming a doctrine-evasion route — addressed by specifying the bypass as a whitelist condition, not a heuristic; (2) section-name drift between the skill's `canonical-isa.md` and `PRDFormat.md` v2.7 — addressed by writing both with the same twelve-section list in the same order this run.

**v6.2.0 in-flight refactor (LATEST as single source of truth):** initially shipped an `AlgorithmVersionSync.hook.ts` to keep `settings.json .pai.algorithmVersion` synced with `DOS/Algorithm/LATEST` on every Write/Edit/MultiEdit. Lucas called Bitter Pill — a hook that synchronizes duplicated state is overhead a smarter design renders unnecessary. Refactored to make LATEST the sole record: statusline reads `cat $PAI_DIR/ALGORITHM/LATEST` directly; `Banner.ts` and `ArchitectureSummaryGenerator.ts::detectAlgorithmVersion()` read LATEST first; `UpdateCounts.ts` no longer mirrors CLAUDE.md regex into settings; `CLAUDE.md MANDATORY FIRST ACTION` became a two-step "Read LATEST → Read v${contents}.md" instruction; `.pai.algorithmVersion` deleted from settings.json; hook + its three Write/Edit/MultiEdit registrations removed. Net change: one source, drift physically impossible. Live-probed end-to-end: bumping LATEST to `6.99.99` flowed through the statusline within one second with no other file touched. Lesson written to v6.2.x doctrine note: when the symptom is "synced field drifted," the right answer is usually to remove the duplicated field, not to add a synchronizer.

**Files updated:**
- NEW: `DOS/Algorithm/v6.2.0.md` (doctrine file)
- NEW: `~/.claude/skills/prd/` (eleven files: SKILL.md, four Examples, six Workflows)
- NEW: `~/.claude/PRD.md` (dog-food: PAI-as-project PRD, seeded via the Seed workflow)
- EDIT: `DOS/Algorithm/LATEST` → `6.2.0`
- EDIT: `DOS/Algorithm/changelog.md` (this entry, plus Index demotion of v6.1.0 to Prior)
- EDIT: `PAI/DOCUMENTATION/PRDFormat.md` → v2.7 (twelve-section schema + tier completeness gate + ID-stability rule + ephemeral pattern + C/R/L format)
...
- EDIT: `DOS/Algorithm/capabilities.md` (Skill("prd") added to OBSERVE-phase capabilities)
- EDIT: `DOS/Algorithm/mode-detection.md` (E1 fast-path bypass specified as whitelist, prd skill invocation mandatory at E2+)

**v6.2.x deferred (automation surface):**
- `hooks/lib/prd-utils.ts` parser updates for `<project>/PRD.md` discovery and twelve-section parsing
- `hooks/PRDSync.hook.ts` extension to project PRDs
- `hooks/CheckpointPerISC.hook.ts` extension to project PRDs
...
- `PAI/PULSE/Observability/observability.ts` rendering for project PRDs
- `~/.claude/skills/prd/Tools/*.ts` CLI implementations (Scaffold.ts, Interview.ts, Completeness.ts, Reconcile.ts, Append.ts)
- Project-PRD seeding migration for the ~30 projects in PROJECTS.md (lazy-seed on first task per Lucas's Q5 answer)

**Migration command (rollback to v6.1.0):**
```bash
echo 6.1.0 > ~/.claude/DOS/ALGORITHM/LATEST && \
...
# The prd skill stays on disk and is harmless under v6.1.0 (skill is invocation-agnostic)
# PRDFormat.md v2.7 is additive — v6.1.0 still parses correctly
# AlgorithmSystem.md is documentation-only — v6.1.0 doesn't depend on it
```

### v5.5.0 → v6.0.0

v6.0.0 is a **frame-shift release** in two coupled moves: (1) closing the v5.0.0 BPE under-cut at the mode-selection layer, and (2) elevating the PRD from "task artifact" to "universal primitive with five identities." Both moves were triggered by the same Lucas-led session — the mode-selection regression was the symptom that forced the deeper architectural conversation.
...
- EDIT: `DOS/Algorithm/LATEST` → `6.0.0`
- EDIT: `DOS/Algorithm/changelog.md` (this entry, plus Index demotion of v5.5.0 to Prior)
...
```bash
echo 5.5.0 > ~/.claude/DOS/ALGORITHM/LATEST && \
...
**Provenance:** Direct Lucas-led session 2026-04-27 (this run). Triggered by an under-escalation event in the same session: Lucas asked "where in our ISCs do standing invariants live?" and Durante answered in NATIVE mode with a 1-paragraph response, when the question genuinely needed E4/E5 doctrinal-design treatment. Root-cause analysis traced the regression to v5.0.0 BPE leaving the mode-selection layer un-floored. The conversation that followed surfaced the deeper insight: the answer to Lucas's original question wasn't to add a parallel acceptance framework — it was to recognize that the PRD already IS the test harness, and the v5.0.0 cut wasn't the only thing missing. Two coupled fixes from one session.
...
The visible win: PRD bodies stop alternating two numbering pools (`ISC-1, ISC-2, ISC-A-1, ISC-3..ISC-12, ISC-A-2, ISC-13...`) and read as one continuous count.

**Migration / parser compatibility.** Both parsers (`hooks/lib/prd-utils.ts` and `DOS/Tools/algorithm.ts`) now detect anti-criterion via `Anti:` prefix on the trimmed description. The legacy `id.includes('-A-')` test is retained as a backward-compat fallback so historical PRDs in `MEMORY/WORK/` still classify correctly. Domain-prefixed IDs (`ISC-CLI-3`, `ISC-TIER-1`, etc.) are unaffected — the dropped namespace was only the single-letter `-A-` anti-criterion convention. Historical PRDs are not migrated (frozen records, same precedent as v5.3.0).

**Files updated:**
- `DOS/Algorithm/v5.5.0.md` (new doctrine; v5.4.0 copied with the Anti-criterion table row + provenance note rewritten)
- `DOS/Algorithm/LATEST` → `v5.5.0`
- `PAI/DOCUMENTATION/PRDFormat.md` (anti-criterion examples + Tags table now show `ISC-N: Anti:`)
- `PAI/DOCUMENTATION/Algorithm/AlgorithmSystem.md` (ISC Quality System Anti-criterion line)
- `DOS/Algorithm/optimize-loop.md` (Guard Rails + Eval mode guard rails examples renumbered)
- `DOS/Tools/algorithm.ts` (line 558 anti-criterion detector switched to description-prefix; line 717 comment example updated)
- `hooks/lib/prd-utils.ts` (line 243 detector switched to `Anti:` prefix with legacy fallback; comments updated)
...
**What changed (and what didn't).** Four tags (`[F]` Functional, `[S]` Structural, `[B]` Behavioral, `[E]` Edge) were pure description: a competent reader infers category from the criterion text in well under a second. Removing them costs nothing and saves visual noise on every PRD Lucas ever reads again. Two tags (`[N]` Negative, `[A]` Antecedent) carried doctrinal weight as gates (≥1 anti-criterion always; ≥1 antecedent when goal is experiential) — but the *gates*, not the *labels*, are load-bearing. The gates survive. Anti-criteria already use `ISC-A-N` numbering and an `Anti:` prose prefix; the `[N]` tag was triple-redundant. Antecedent ISCs now use an `Antecedent:` prose prefix in the criterion description.
...
**What did NOT change:** every other v5.2.0 doctrine. Seven phases, Hard-to-Vary Doctrine preamble, PRD as system of record, the variation test, PRD-Level HVA (Coverage/Tightness/Uniqueness), per-ISC HVA with cluster-bearing rule, Verification Doctrine (Rules 1/2/2a/3 + P1-P5 hardening), `[DEFERRED-VERIFY]` ISC status, Inline Verification mandate, Reproduce-First blocking gate, Intent Echo, Reverse Engineering block, Preflight Gates A-D, Feedback Memory Auto-Consult, Deliverable Manifest, Parallelism Opportunity Scan, Re-Read Check, Fluff Sweep, Euphoric Surprise Prediction, Knowledge Capture, Documentation Sync, Forge/Anvil/Cato bindings, voice-curl format, frontmatter contract, fast-path trigger, the 7/7 SUMMARY block as terminator, the reflection JSONL schema, the per-tier ISC count floors at E2+ (16/32/128/256), and the CheckpointPerISC hook. The two doctrinal minimums also survive verbatim — only their *expression* moves from a bracket letter to a prose prefix.

**Migration / parser compatibility.** The parser (`hooks/lib/prd-utils.ts`) is **backward-compatible**: legacy PRDs with `[F]`/`[S]`/`[B]`/`[N]`/`[E]`/`[A]` still parse correctly and the `category` field is retained on `CriterionEntry` for them. New PRDs simply omit the bracket. Hooks (PRDSync, CheckpointPerISC) require no changes — they consume `parseCriteriaList()` which now accepts both shapes. Historical PRDs in `MEMORY/WORK/` are not migrated (they are frozen records).

**Files updated:**
- `DOS/Algorithm/v5.3.0.md` (new doctrine file, copy of v5.2.0 with category section rewritten)
- `DOS/Algorithm/LATEST` → `v5.3.0`
- `PAI/DOCUMENTATION/PRDFormat.md` (Tags section + format examples + doctrinal-minimums language)
...
- `hooks/lib/isa-template.ts` (PRD stub example uses bare `ISC-N:` format)
- `hooks/lib/prd-utils.ts` (regex made format-agnostic; comments updated; `VALID_CATEGORIES` retained for legacy parse, marked `@deprecated`)
...
**Soft, not strict.** The floor is a coverage anchor, not a blocking ceremony. If the task surface genuinely produces fewer atomic ISCs after the granularity rule has been applied honestly, document the under-decomposition and the reason in `## Decisions` and proceed. The ILA Coverage axis at E2+ remains the structural enforcer — the floor surfaces an under-decomposed PRD earlier, before THINK.
...
**Migration:** v5.2.0 lives alongside v5.1.0 and v5.0.0. To roll back: `echo v5.1.0 > LATEST && jq '.pai.algorithmVersion="5.1.0"' ~/.claude/settings.json | sponge ~/.claude/settings.json && sed -i '' 's|v5.2.0.md|v5.1.0.md|' ~/.claude/CLAUDE.md`. Hooks unchanged (parse only `effort` enum + ISC checkboxes + category tags). Floors are doctrine-level only — `prd-utils.ts` does not enforce them; the Algorithm prompt does.

### v5.0.0 → v5.1.0

v5.1.0 adds **per-step durability** — the one design move PAI hadn't yet committed to. The `CheckpointPerISC` hook (PostToolUse Edit/Write/MultiEdit on `MEMORY/WORK/{slug}/PRD.md`) auto-commits a git checkpoint in every opted-in repo on each `[ ]`→`[x]` ISC transition. Commits use `--no-verify --no-gpg-sign` to avoid husky/GPG hangs that would block the session. Sidecar state at `MEMORY/WORK/{slug}/.checkpoint-state.json` makes commits idempotent (no double-commit). Allowlist at `~/.claude/checkpoint-repos.txt` — defaults to `~/.claude` only; other repos require explicit Lucas opt-in (no surprise commits in WIP branches). Inspection + rollback via `bun ~/.claude/DOS/TOOLS/Checkpoint.ts {list|show|rollback} <slug> [<isc-id>]`. **Rollback is preview-only** — prints the suggested `git reset --hard <sha>` per repo and exits; never executes destructive ops without Lucas running them himself. Doctrine elsewhere is unchanged from v5.0.0; the only addition is the **Checkpoints subsection** in EXECUTE below. Provenance: R1 from session `20260426-230039_hankweave-pai-improvements-analysis` — Hankweave's per-codon git checkpoint absorbed as a PAI-native primitive without adopting Hankweave's runtime.
...
- 2026-05-04 | conjectured: "five parallel teammates with file-ownership partition is sufficient coordination; convention compliance is implicit"
  refuted_by: 6 new *.hook.ts files authored 644 (not 755) by all 5 streams independently; Lucas hit Permission denied at hook fire time
  learned: tribal-knowledge conventions don't survive parallel delegation. Five independent agents reading the codebase will not converge on `chmod 755` unless it appears IN the Pre-Delegation Contract OR is enforced by a post-write hook
  criterion_now: any future Pre-Delegation Contract that includes `*.hook.ts` files MUST list "chmod 755 after Write" in the per-stream checklist; OR settings.json gains a PostToolUse hook that auto-chmods *.hook.ts on Write

- 2026-05-04 | conjectured: "completing all PRD criteria and running the parent-repo commit is sufficient to ship a LEARN phase — the submodule content follows automatically"
  refuted_by: audit (MEMORY/ARCHIVE/2026-05-04_v007-test-transcript/audit-uncommitted-files.txt) found that Releases/v0.0.6/.claude and Platform/studio submodules carried uncommitted working-tree edits after every parent commit; cleanup required two separate commits (7186e77 + 1af5c5dd) to surface the lost work
  learned: every phase=complete needs a working-tree-clean check across parent + all submodules before the PRD frontmatter is stamped; parent-centric commits are structurally blind to submodule dirt
  criterion_now: WORKING-TREE-CLEAN GATE rule in LEARN phase of v0.0.7.md — `bun ~/Durante/Tools/working-tree-clean-gate.ts --format json` must exit 0 before `phase: complete` is written; dirty blocks the transition; `DOS_ALLOW_DIRTY_LEARN=1` is the only bypass with audited stderr WARNING

- 2026-07-04 | conjectured: "shipping the RFC-0149 task-projection machinery (sub-doc `task-projection.md` + TaskEventCapture hook + advisory parity lint) was enough for Durante to use the Claude Code Task tools"
  refuted_by: the operator reported "I still cannot see Durante using Tasks" — nothing in the operative doctrine ever fired the projection. `dag-playbook.md` is used only because §6.3 PLAN explicitly points to it; `task-projection.md` had NO phase-text pointer and was absent from the §9 sub-docs table, so no session ever loaded it and none projected.
  learned: a sub-doc with no phase-text pointer is unreachable wallpaper — the same 121:0 advisory-graveyard failure in a new shape. Reachability (a §9 row + a §6.3 pointer) is the load-bearing wire, not the sub-doc's existence.
  criterion_now: §6.3 PLAN gains a TASK PROJECTION substep (fires by default at the PLAN→EXECUTE boundary for effort ≥ E2 OR teammate runs, emits a `📌 TASK PROJECTION:` line) + a §9 `task-projection.md` row; the miss is observed warn-tier by `Tools/task-projection-check.ts` + its Sentinel companion — never a §6.6 fail-closed gate (RFC-0149 §5). Council+RedTeam ratified 2026-07-04 (PRD 20260704-004247).
