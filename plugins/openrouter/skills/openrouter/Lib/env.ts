...
export function requireStudioApiUrl(): string {
  // studioClient:exempt — public OpenRouter models route requires only STUDIO_API_URL; STUDIO_API_KEY is intentionally optional.
  const apiUrl = process.env.STUDIO_API_URL?.replace(/\/+$/, '');
  if (apiUrl) return apiUrl;
  throw new MissingStudioEnvError('STUDIO_API_URL');
}
