...
import { VOICE_BRIDGE_URL } from '../../DOS/Tools/voice-constants';
import { join, basename, dirname } from 'path';
import { dosPath, getDosDir } from '../lib/paths';
import { getIdentity } from '../lib/identity';
import { inference } from '../../DOS/Tools/Inference';
import type { ParsedTranscript } from '../../DOS/Tools/TranscriptParser';
...
const SYSTEM_DIR = dosPath('DOS');
const HOOKS_DIR = dosPath('hooks');
const HANDLERS_DIR = join(HOOKS_DIR, 'handlers');
const LIB_DIR = join(HOOKS_DIR, 'lib');
const DRIFT_STATE_FILE = dosPath('MEMORY', 'STATE', 'doc-drift-state.json');
const REVIEW_QUEUE_FILE = dosPath('MEMORY', 'STATE', 'doc-review-queue.json');
...
function isSystemDocModified(modifiedFiles: Set<string>): boolean {
  for (const path of modifiedFiles) {
    if (path.includes('DOS/') && path.endsWith('.md')) return true;
  }
  return false;
}
...
/**
 * Check if ANY meaningful DOS system file was modified.
...
function isSystemFileModified(modifiedFiles: Set<string>): boolean {
  ...
  for (const filePath of modifiedFiles) {
    ...
    const relPath = filePath.startsWith(DOS_DIR)
      ? filePath.slice(DOS_DIR.length + 1)
      : filePath;

    // Must be within DOS directory
    if (filePath.startsWith('/') && !filePath.startsWith(DOS_DIR)) continue;

    // Skip excluded paths
    if (EXCLUDED.some(ex => relPath.includes(ex))) continue;

    // Match meaningful system files (DOS root MDs, SYSTEM docs, USER dir, Algorithm, Tools, Workflows, skills)
    if ((relPath.startsWith('DOS/') || relPath.includes('skills/')) && (relPath.endsWith('.md') || relPath.endsWith('.ts') || relPath.endsWith('.yaml') || relPath.endsWith('.yml'))) return true;
    if (relPath.includes('hooks/') && relPath.endsWith('.ts')) return true;
    if (relPath.endsWith('settings.json')) return true;
    if (relPath.includes('DOS/Algorithm/') && relPath.endsWith('.md')) return true;
...
function checkSystemDocRefs(docsToCheck: string[], systemDocsOnDisk: Set<string>): DriftItem[] {
  const drift: DriftItem[] = [];
  // Match backtick-wrapped or plain doc references in DOS/ (both old skills/DOS/ and new DOS/ paths)
  const sysDocRefRegex = /(?:`|'|")(?:~\/\.claude\/)?(?:skills\/)?DOS\/([\w/]+\.md)(?:`|'|")/g;
  ...
  for (const docFile of docsToCheck) {
    ...
    while ((match = sysDocRefRegex.exec(content)) !== null) {
      const refTarget = match[1]; // e.g., "DOSSYSTEMARCHITECTURE.md" or "DOSSECURITYSYSTEM/ARCHITECTURE.md"
      const targetBasename = basename(refTarget);
      // For nested paths like DOSSECURITYSYSTEM/ARCHITECTURE.md, check file exists
      const targetPath = join(SYSTEM_DIR, refTarget);
      if (!existsSync(targetPath)) {
        drift.push({
          doc: docFile,
          pattern: 'system_doc_ref',
          reference: `DOS/${refTarget}`,
          issue: `References "DOS/${refTarget}" but file does not exist`,
        });
      }
    }
  }
...
/**
 * Doctrine files the LLM is forbidden from auto-editing.
 *
 * 2026-05-05 incident: LLM repeatedly "fact-checked" CLAUDE.md by hardcoding
 * `DOS/Algorithm/v0.0.7-enhanced.md.md` (double `.md` typo) into the doctrine
 * line, encoding a downstream concatenation bug at `RebuildDOS.ts:79` as
 * canonical doctrine. Root cause was a drift in `LATEST` (fixed in commit
 * `7ba2da6`), but the LLM kept regenerating the broken edit even after the
 * upstream cause was removed.
 *
 * CLAUDE.md is the operative doctrine file — it must stay under deliberate
 * human authorship, not LLM-driven find-and-replace. Surfaced doctrinal drift
 * still gets logged for human review via the review queue, just not
 * auto-applied.
 */
const NEVER_AUTO_EDIT = new Set<string>([
  'CLAUDE.md',
]);
 ...
 *
 * Skips files in NEVER_AUTO_EDIT (doctrine-protection); those edits are
 * surfaced via the review queue instead.
 */
function applyInferenceEdits(edits: InferenceEdit[]): string[] {
  const applied: string[] = [];

  for (const edit of edits) {
    if (NEVER_AUTO_EDIT.has(edit.doc)) {
      console.error(`${TAG} [INFERENCE-APPLY] ${edit.doc} is doctrine-protected — skipping LLM edit, queuing for review: ${edit.reason}`);
      continue;
    }
...
export async function handleDocCrossRefIntegrity(
  ...
  const allDrift = runAllPatternChecks(docsToCheck, hooksOnDisk, handlersOnDisk, libsOnDisk, systemDocsOnDisk);

  const updatesApplied = applyDeterministicUpdates(modifiedFiles, hooksOnDisk.size, hasHookChanges);

  const inferenceEdits = await maybeRunInference(allDrift, modifiedFiles, docsToCheck, updatesApplied);

  saveDriftReport(hookInput.session_id, docsToCheck, allDrift, updatesApplied);

  if (allDrift.length > 0) {
    addToReviewQueue(allDrift);
  }

  logSummary(handlerStart, docsToCheck.length, allDrift, updatesApplied, inferenceEdits);

  await maybeNotifyVoice(updatesApplied, hasHookChanges, hasDocChanges);
}

// ============================================================================
// Step-down helpers (file-local, called only from handleDocCrossRefIntegrity)
// ============================================================================

function logPatternResult(label: string, okLabel: string, drift: DriftItem[], allDrift: DriftItem[]): void {
  if (drift.length > 0) {
    console.error(`${TAG} [DRIFT] ${label}: ${drift.length} broken refs found`);
    for (const item of drift) {
      console.error(`${TAG}   - ${item.doc}: ${item.issue}`);
    }
    allDrift.push(...drift);
  } else {
    console.error(`${TAG} [OK] ${okLabel}`);
  }
}

function runAllPatternChecks(
  docsToCheck: string[],
  hooksOnDisk: Set<string>,
  handlersOnDisk: Set<string>,
  libsOnDisk: Set<string>,
  systemDocsOnDisk: Set<string>,
): DriftItem[] {
  const allDrift: DriftItem[] = [];
  logPatternResult('Hook file references', 'Hook file references: all valid', checkHookFileRefs(docsToCheck, hooksOnDisk), allDrift);
  logPatternResult('Handler file references', 'Handler file references: all valid', checkHandlerFileRefs(docsToCheck, handlersOnDisk), allDrift);
  logPatternResult('Lib file references', 'Lib file references: all valid', checkLibFileRefs(docsToCheck, libsOnDisk), allDrift);
  logPatternResult('System doc references', 'System doc references: all valid', checkSystemDocRefs(docsToCheck, systemDocsOnDisk), allDrift);
  logPatternResult('Hook counts', 'Hook counts: accurate', checkHookCounts(docsToCheck, hooksOnDisk.size), allDrift);
  return allDrift;
}

function applyDeterministicUpdates(
  modifiedFiles: Set<string>,
  hookCount: number,
  hasHookChanges: boolean,
): string[] {
  const updatesApplied: string[] = [];

  for (const path of modifiedFiles) {
    if (path.includes('DOS/') && path.endsWith('.md')) {
      const result = updateLastUpdatedTimestamp(basename(path));
      if (result) {
  ...
  if (hasHookChanges) {
    const countResult = updateHookCount(hookCount);
    if (countResult) {
...
async function maybeRunInference(
  ...
  if (inferenceEdits.length > 0) {
    updatesApplied.push(...applyInferenceEdits(inferenceEdits));
  } else {
    console.error(`${TAG} [INFERENCE] No semantic corrections needed`);
  }
  return inferenceEdits;
}

function saveDriftReport(
  sessionId: string,
  docsChecked: string[],
...
function logSummary(
  ...
  console.error(`${TAG} Docs checked: ${docsCheckedCount}`);
...
async function maybeNotifyVoice(
  updatesApplied: string[],
  hasHookChanges: boolean,
  hasDocChanges: boolean,
): Promise<void> {
  // No voice for "queued for review" or "in sync" — that's noise
  if (updatesApplied.length === 0) return;

  // Delay 3s so the main voice line plays first
