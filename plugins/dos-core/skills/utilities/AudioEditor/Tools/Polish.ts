...
// ============================================================================
// Environment Loading — keys from ~/.claude/.gateway.env
// ============================================================================
...
// TODO: Add Cleanvoice Studio gateway route, then remove BYOK path
const apiKey = process.env.CLEANVOICE_API_KEY;
if (!apiKey) {
  console.error("CLEANVOICE_API_KEY not found. Set it in ~/.claude/.gateway.env");
...
for (let i = 0; i < MAX_POLLS; i++) {
  ...
  if (status === "completed" || status === "done") {
    ...
    await writeArtifact(outFile, new Uint8Array(outputData), {
