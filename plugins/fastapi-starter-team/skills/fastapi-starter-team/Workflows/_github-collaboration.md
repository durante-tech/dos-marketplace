...
Shared substrate for every FastAPIStarterTeam workflow that interacts with a PR over time — not just the one-shot ship. Codifies the team's discipline: keep TODOs current, engage with reviewers, post status when work meaningfully advances, keep the PR body fresh.

**Source of truth:**
- `Workflows/_pr-loop-shared.md` (safety tier table — Auto-read / Auto-write low-blast / Gated / NEVER — preserved by reference)
- `Workflows/_commit-merge.md` (commit/merge format + PR creation template — preserved by reference)
- `~/Developer/dos-fastapi-starter/AGENTS.md` (concision + Hard Rule 8 no emojis — applies to comment text too)

**Sibling:** `MakerkitTeam/Workflows/_github-collaboration.md`. Same shape; reflavored references (FastAPIStarterTeam name, starter AGENTS.md path).
...
For inline diff comments (the threaded discussion on lines of code):

```bash
gh api repos/{owner}/{repo}/pulls/{N}/comments
```

Incorporate prior discussion before adding new commits / TODOs / comments. Silent override of reviewer feedback is an anti-pattern.

## TODO State Management

The artifact JSON at `MEMORY/ARTIFACTS/fastapistarter-pr-{N}-todos.json` is the source of truth; the PR comment is the rendered view (per `_pr-loop-shared.md` Artifact JSON Schema). Update both in lockstep.
...
Three comment types, each with its own pattern. All three are Auto-write low-blast tier per `_pr-loop-shared.md`.

### Type 1 — Auto-managed TODO comment

Single comment per PR per session, edited via `--edit-last --create-if-none`. The `Tools/RenderTodoComment.ts` renderer produces the deterministic body.
...
This is a SEPARATE comment from the auto-managed Type 1 — it engages a specific reviewer point, not the TODO state. Keep replies short.
...
The `review_comment_id` comes from `gh api repos/{owner}/{repo}/pulls/{N}/comments`. Inline replies stay threaded with the original line discussion.

**Reply timing**: reply within the same FastAPIStarterTeam session if the reviewer's comment is fresh (< 10 min old). For older comments, surface to operator first.
...
```markdown
**Execution status: 4/12 done, 1 blocked, 7 pending.** Last update: 2026-05-06T20:45:00Z by ExecuteOpenTodos. Branch: `fix/pr-42-todos`.
...
Mark Test plan checkboxes `[x]` as items complete in lockstep with TODO checklist updates.

## Label / State Discipline

Per project convention (starter-specific, configurable). Read existing labels first via `gh pr view --json labels` and match the starter's existing taxonomy — never invent labels.
...
If the starter doesn't use these labels, skip silently — the convention is project-set, not enforced here.
...
2. **Stale TODO checklist** — work done in code, but the public comment still says "incomplete". Lockstep rule.
...
5. **Reviewer comment ignored** — no reply at all. Anti-pattern even when we agree silently.
6. **Closing PR without comment** — silent close after issues raised. Always explain in a final comment.
7. **`gh pr review --approve` from team verdict** — Gated tier. Only operator approves.
8. **Posting duplicate replies** — single reply per reviewer point.
9. **Inventing labels** — read existing label set first; match starter taxonomy.
10. **Going silent on multi-batch runs** — the auto-managed TODO comment is the heartbeat.
...
- `Workflows/_test-pyramid-gate.md` — ships-with-tests gate (the "done" condition for batches)
- `Workflows/_algorithm-team-spawn.md` — TeamCreate + Agent(team_name:...) for parallel reviewers
