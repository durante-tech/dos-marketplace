...
function loadConfig(): Config {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('Error: API_KEY not found in ${DOS_DIR}/.env');
    ...
    console.error('Error: Cannot read ${DOS_DIR}/.env');
    console.error('Create file: touch ${DOS_DIR}/.env');
    process.exit(1);
  }
}
```
...
**Key principles:**
- Load from ${DOS_DIR}/.env (DOS standard)
...
CONFIGURATION:
  API Key: ${DOS_DIR}/.env (API_KEY=your_key)
  Base URL: ${DEFAULTS.baseUrl}

PHILOSOPHY:
  ${CLI_NAME} follows DOS's CLI-First Architecture:
...
- [ ] Configuration loading (from ${DOS_DIR}/.env)
