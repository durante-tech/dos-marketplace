/**
 * Remove.bg Gateway Shim — Phase 3
 *
 * Drop-in replacement for direct remove.bg API calls from DOS pack
 * tools. Detects STUDIO_API_URL + STUDIO_API_KEY via loadEnv() and
 * routes through the Studio gateway (`/api/v1/media/removebg/remove`)
 * when both are set. Otherwise falls back to calling api.remove.bg
 * directly using REMOVEBG_API_KEY (BYOK).
 *
 * This shim serves TWO consumers:
 *   1. Packs/media/src/Art/Tools/Generate.ts — removeBackground()
 *   2. Packs/media/src/BackgroundRemoval/Tools/RemoveBg.ts — removeWithAPI()
 *
 * Usage:
 *
 *   import { removeBackgroundViaGateway } from "../../Lib/removebg-gateway.ts";
 *
 *   const { imageBuffer, chargedCredits, mode } = await removeBackgroundViaGateway({
 ...
 *   REMOVEBG_API_KEY        BYOK fallback (ignored in gateway mode)
 *
 * See also:
...
export type RemoveBgMode = "studio";

export interface RemoveBackgroundOptions {
  imageBuffer: Buffer;
  size?: "auto" | "preview" | "hd" | "4k";
}
...
const STUDIO_ROUTE = "/api/v1/media/removebg/remove";

export async function removeBackgroundViaGateway(
  ...
  return removeViaGateway(options, studioUrl, studioKey);
}
...
async function removeViaGateway(
  options: RemoveBackgroundOptions,
  studioUrl: string,
  studioKey: string,
): Promise<RemoveBackgroundResult> {
