---
name: Compare Models
description: Compare multiple models on the same eval use case and rank their results.
status: STABLE
---
...
The shipped eval tools run suites against THE agent (no `--model` flag); cross-model comparison
goes through the **openrouter skill** (Studio-metered gateway, 300+ models) over the rendered
prompt from Step 3:

1. Render the prompt once per test case (RenderTemplate, Step 3).
2. For each candidate model, send the rendered prompt via the openrouter skill's chat completion
   (`USE WHEN model comparison / bakeoff` — see `~/.claude/skills/openrouter/SKILL.md`), capturing
   response, latency, and token usage.
3. Write one `Results/<use-case>/<run-id>/results.json` per model run (same shape as suite runs)
   so ViewResults reads comparisons with the same commands.
4. Grade each capture with the use case's scorers (deterministic first, judges second).
