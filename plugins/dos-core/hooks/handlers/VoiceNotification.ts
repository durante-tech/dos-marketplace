...
const VOICE_LOG_PATH = dosPath('MEMORY', 'VOICE', 'voice-events.jsonl');
const CURRENT_WORK_PATH = dosPath('MEMORY', 'STATE', 'current-work.json');
...
function getActiveWorkDir(): string | null {
  try {
    ...
    if (state.work_dir) {
      const workPath = dosPath('MEMORY', 'WORK', state.work_dir);
...
function logVoiceEvent(event: VoiceEvent): void {
  ...
  try {
    const dir = dosPath('MEMORY', 'VOICE');
    if (!existsSync(dir)) {
      mkdirSync(dir, { recursive: true });
    }
    rotateIfNeeded(VOICE_LOG_PATH);
...
async function sendNotification(payload: ElevenLabsNotificationPayload, sessionId: string): Promise<void> {
  ...
  try {
    ...
    const response = await fetch(VOICE_BRIDGE_URL, {
      method: 'POST',
