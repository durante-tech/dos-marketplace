...
description: Launches multiple parallel general-purpose agents for grunt work — same task across different inputs, model-tiered by complexity, always followed by a mandatory spotcheck.
status: STABLE
bestPath:
  - title: "Identify Task List"
    description: "Extract the list of items (companies, files, URLs) to process in parallel."
  - title: "Create Task-Specific Prompts"
    description: "Build a detailed prompt per agent with full context and a timing scope section."
  - title: "Launch All Agents in a Single Message"
    description: "Send every Task call together for true parallel, simultaneous execution."
  - title: "Spotcheck Results"
    description: "Launch a mandatory follow-up agent to check consistency across all results."
---
...
<!-- partial: _workflow-voice.md skill_name=Agents workflow_name=SpawnParallelAgents action_phrase=" to launch agents" -->
