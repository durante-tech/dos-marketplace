...
- You want to use Nano Banana Pro's full capabilities
...
**Infographics use the EXCALIDRAW whiteboard engineering-drawing aesthetic** — drafted technical with measured boxes, sketchy-illustrative lines, and measured organic shapes. This is the same style as mermaid.md technical diagrams but with richer graphics and narrative.
...
- WOBBLY BOXES — rectangles with rough, drafted technical edges (not perfect)
...
- RICH GRAPHICS — icons, illustrations, visual metaphors (all sketchy-illustrative)
```

### What Makes a Good Excalidraw Infographic

1. **Hand-Drawn Feel** — Everything looks sketched, not digital
2. **Measured Shapes** — No perfect rectangles, circles, or lines
3. **Rich Graphics** — Icons and illustrations in sketchy-illustrative style
4. **Visual Narrative** — Panels flow and tell a story
5. **Strategic Color** — Orange/navy accents on key elements, mostly black
...
- Hot Accent Orange (#F97316): Critical elements, key stats, title (10-20%)
- Blueprint Navy (#1E3A8A): Secondary highlights (5-10%)
- Blueprint Navy (#1E3A8A): All text labels
...
```
DEFAULT: Warm-White Paper/Sepia #FEF7E0 (matches blog aesthetic)
...
**Warm-White Paper (#FEF7E0) is the DEFAULT background.** Only use white (#FFFFFF) if the user explicitly requests it.

**For transparent background** — use the **Images skill** for background removal:

```bash
bun ~/.claude/DOS/Tools/RemoveBg.ts /path/to/visualization.png
```
...
```
Excalidraw-style whiteboard infographic with rich drafted technical graphics.

STYLE: Excalidraw whiteboard engineering-drawing aesthetic
- Measured rectangles with rough edges (not perfect boxes)
- Sketchy arrows with slight wobble (not ruler-straight)
- Measured shapes throughout (circles slightly oval)
- Drafted monospace text labels (natural slant, measured)
- Variable line weight (boxes thicker, details thinner)
- Whiteboard/sketch paper feel

BACKGROUND: Warm-White Paper #FEF7E0 (DEFAULT) — only use White #FFFFFF if explicitly requested
...
- Color: Hot Accent Orange #F97316 (or Black #000000)
...
- Drafted technical icons and illustrations (measured, organic)
- Data visualized with sketchy-illustrative charts/graphics
- Panels as measured boxes with headers
- Flow shown with sketchy-illustrative arrows

COLOR USAGE:
- Black: All primary structure and most elements
- Orange: Title, key stats, critical accents
- Navy: Secondary highlights
- Navy: All body text

CRITICAL:
- Excalidraw drafted technical whiteboard aesthetic throughout
- All shapes measured, all lines measured
...
- **Hot Accent Orange #F97316:** Critical insights, key data points, optimal zones
- **Blueprint Navy #1E3A8A:** Secondary data, supporting elements, context
- **Blueprint Navy #1E3A8A:** All body text and labels
- **Background:** Warm-White Paper #FEF7E0 (DEFAULT — only use white if explicitly requested)

Strategic color use:
- Don't color everything
- Orange for "look here" moments
- Navy for supporting information
...
COLOR CODING:
Orange: [Specific elements to highlight]
Navy: [Supporting elements]
...
**Build the generation prompt using Nano Banana Pro's capabilities:**
...
BACKGROUND: Warm-White Paper #FEF7E0 (DEFAULT) — only use White #FFFFFF if user explicitly requests it

AESTHETIC:
- Professional infographic quality (deliverable standard)
- Engineering blueprint elements where appropriate
...
- Color: Hot Accent Orange #F97316 (insights) or Blueprint Navy #1E3A8A (technical notes)
...
- Style: [Precise data viz / Engineering blueprint / Hybrid]
- Color: Black structure, Orange highlights on [specific], Navy on [specific]
...
- Style: [Drafted technical / Iconic / Metaphorical]
- Color: Black linework, Orange accents on [specific]
...
- Hot Accent Orange (#F97316): [Critical data points, key insights, optimal zones]
- Blueprint Navy (#1E3A8A): [Secondary data, supporting elements]
- Blueprint Navy (#1E3A8A): [All text labels and annotations]
- Background: Warm-White Paper #FEF7E0 (DEFAULT — white only if explicitly requested)
...
- Engineering blueprint quality where appropriate
...
bun Tools/dos-image.ts "[YOUR COMPREHENSIVE PROMPT]" \
  ...
  --size=1024x1024 \
  --telemetry-tag=Media/Art/Visualize
```

**Why `--intent=diagram` for this workflow:** the Port routes diagram to GPT-Image-1, which has the best text rendering among image models — critical for infographics, complex multi-element compositions, and data visualization labels. Same property the prior `--model nano-banana-pro` choice was selecting for, expressed as intent rather than vendor.

**Advanced fallthrough — Generate.ts CLI for `--thumbnail` / `--creative-variations` flags:**

```bash
# Blog-header thumbnail variant (transparent + colored background)
...
The `--thumbnail` and `--creative-variations` flags don't surface through the Port today; use the CLI form when those flows are needed. Phase 5 may add post-processing flags to the Port surface.
...
- [ ] **brand aesthetic maintained:** Flat colors, appropriate drafted technical vs precise
- [ ] **Typography hierarchy clear:** 3 tiers obviously distinct
- [ ] **Color strategic:** Orange/navy highlight key elements, not overwhelming
...
| **Message unclear** | No clear visual hierarchy | Strengthen primary element (make larger, add orange), reduce secondary |
...
| **Looks generic** | Missing brand aesthetic | Add engineering blueprint elements, strategic orange/navy, flatten any gradients |
| **Elements disconnected** | Poor composition | Redesign spatial layout, add visual connectors (arrows, borders, grouping) |
| **Color chaos** | Too much color everywhere | Limit orange to 2-3 key elements, navy to 1-2 supporting, rest black/navy |
| **Not professional** | Too sketchy-illustrative or too rigid | Balance: data viz precise, editorial elements drafted technical, clean typography |
...
**Example:** "78 out of 100 developers" shown as 78 orange icons + 22 gray icons
**Aspect:** 1:1 or 4:3

### Pattern 5: Annotated Data Story
**When:** Data needs narrative explanation
**Layout:** Primary chart with drafted technical annotations explaining insights
**Example:** Timeline chart with orange arrows: "*this is when everything changed*"
**Aspect:** 16:9 or 21:9
...
4. **Color discipline** — Orange/navy strategic, not everywhere
