...
/**
 * Resolve the MEMORY/STATE directory that holds the entity index, mirroring
 * getMemorySubdir() semantics locally — project-first, then cwd, then the
 * global ~/.claude fallback. Implemented inline (no hooks-lib import) so the
 * pack stays independently buildable/distributable.
 *
 * UTIL-06: the index used to live in the pack's own source dir
 * (__dirname-relative), which pollutes a read-only marketplace install and
 * gives every project a single shared, unscoped index.
 */
function resolveStateDir(): string {
  const projectDir = process.env.CLAUDE_PROJECT_DIR;
  if (projectDir) {
    const candidate = path.join(projectDir, 'MEMORY', 'STATE');
    if (existsSync(candidate)) return candidate;
  }
  const cwdCandidate = path.join(process.cwd(), 'MEMORY', 'STATE');
  if (existsSync(cwdCandidate)) return cwdCandidate;
  return path.join(os.homedir(), '.claude', 'MEMORY', 'STATE');
}

// Memoized (code-review 2026-07-07): resolution is existence-dependent, so an
// unmemoized load→save cycle could resolve to TWO different dirs if a hook
// creates $CLAUDE_PROJECT_DIR/MEMORY/STATE mid-run — silently forking the
// index. One resolution per process.
let cachedIndexPath: string | null = null;
function getEntityIndexPath(): string {
  if (cachedIndexPath === null) {
    cachedIndexPath = path.join(resolveStateDir(), 'parser-entity-index.json');
  }
...
export async function loadEntityIndex(): Promise<EntityIndex> {
  try {
    const data = await fs.readFile(getEntityIndexPath(), 'utf-8');
...
export async function saveEntityIndex(index: EntityIndex): Promise<void> {
  ...
  // Ensure the MEMORY/STATE dir exists (created on demand, like the hooks).
  await fs.mkdir(path.dirname(indexPath), { recursive: true });

  // Write to temp file first
  // writeArtifact:exempt — index tmp write (atomic rename follows)
