...
`~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/FirstPrinciples/`
...
**Self-differentiation:** Unlike RedTeam (attacks to break) and Council (voices debating), FirstPrinciples decomposes to axioms and reconstructs from verified truths. It OWNS *challenge assumptions / constraints* in the decompose sense; RedTeam composes `FirstPrinciples/Challenge` for its assumption-attacks — the shared word "challenge" is a composition contract, not a collision.
