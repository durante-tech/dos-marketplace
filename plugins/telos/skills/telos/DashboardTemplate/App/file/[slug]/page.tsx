  ...
  useEffect(() => {
    ...
    fetch('/api/files/count')
      ...
      .then(data => {
        ...
        if (!matchingFile) {
          // No file matches this slug — surface a not-found state instead of
          // an indistinguishable, permanent "Loading..." screen.
          setLoadError(true)
          return
        }

        // Fetch the actual file content
        return fetch(`/api/file/get?filename=${encodeURIComponent(matchingFile)}`)
          .then(res => res.json())
          .then(fileData => {
            setFile({
              name: slug,
  ...
  if (loadError) {
    return (
      <div className="p-8">
        ...
        <p className="text-gray-600">No file matches "{slug}", or it could not be loaded.</p>
      </div>
    )
  }
...
// Minimal CSV line parser that respects double-quoted fields (RFC 4180):
// a comma inside quotes does not split, and "" is an escaped quote.
