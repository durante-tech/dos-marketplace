...
description: Convert content to programmatic MP4 video via Remotion with React-based composition
status: BETA
featured: true
icon: Film
bestPath:
  - title: "Content Analysis"
    description: "Parse and structure source content into visual narrative segments."
  - title: "React Composition"
    description: "Build Remotion component tree with timing, transitions, and layout."
  - title: "Animation Choreography"
    description: "Sequence motion, easing, and visual effects across the timeline."
  - title: "MP4 Render"
    description: "Render final video output with optimized encoding and resolution."
---

# ContentToAnimation Workflow

Transform any content into professional DOS-themed animations.
...
**MANDATORY: Apply DOS Theme**
```typescript
import { DOS_THEME } from '~/.claude/skills/media/Remotion/theme'

// All components MUST use:
// - DOS_THEME.colors for all colors
// - DOS_THEME.typography for text styles
// - DOS_THEME.animation for spring configs
// - DOS_THEME.spacing for layout
```
...
const TitleScene: React.FC<{ title: string; subtitle?: string }> = ({ title, subtitle }) => {
  ...
  const titleScale = spring({ frame, fps, config: DOS_THEME.animation.springDefault })
  const subtitleOpacity = interpolate(frame, [20, 50], [0, 1], { extrapolateRight: 'clamp' })

  return (
    <AbsoluteFill style={{
      backgroundColor: DOS_THEME.colors.background,
      justifyContent: 'center',
      alignItems: 'center',
    }}>
      <h1 style={{
        ...DOS_THEME.typography.title,
        color: DOS_THEME.colors.accent,
      ...
      {subtitle && (
        <p style={{
          ...DOS_THEME.typography.subtitle,
          color: DOS_THEME.colors.textMuted,
...
const KeyPointsScene: React.FC<{ heading: string; points: string[] }> = ({ heading, points }) => {
  ...
  return (
    <AbsoluteFill style={{
      backgroundColor: DOS_THEME.colors.background,
      padding: DOS_THEME.spacing.page,
    }}>
      <h2 style={{
        ...DOS_THEME.typography.heading,
        color: DOS_THEME.colors.text,
        opacity: interpolate(frame, [0, 20], [0, 1], { extrapolateRight: 'clamp' }),
        marginBottom: DOS_THEME.spacing.section,
      }}>
        {heading}
      </h2>

      {points.map((point, i) => {
        const delay = 20 + (i * DOS_THEME.animation.staggerDelay)
        const opacity = interpolate(frame, [delay, delay + 20], [0, 1], { extrapolateRight: 'clamp' })
        const x = interpolate(frame, [delay, delay + 20], [-30, 0], { extrapolateRight: 'clamp' })

        return (
          <div key={i} style={{
            ...DOS_THEME.typography.body,
            color: DOS_THEME.colors.text,
            opacity,
            transform: `translateX(${x}px)`,
            marginBottom: DOS_THEME.spacing.element,
            display: 'flex',
            alignItems: 'flex-start',
          }}>
            <span style={{ color: DOS_THEME.colors.accent, marginRight: 16 }}>✓</span>
            {point}
          </div>
        )
      })}
...
const DataScene: React.FC<{ data: { label: string; value: string }[] }> = ({ data }) => {
  ...
  return (
    <AbsoluteFill style={{
      ...
      alignItems: 'center',
      flexDirection: 'row',
      gap: DOS_THEME.spacing.section,
    }}>
      {data.map((item, i) => {
        const delay = i * 15
        const scale = spring({ frame: Math.max(0, frame - delay), fps, config: DOS_THEME.animation.springBouncy })
...
- Load: `~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/Art/PREFERENCES.md`
