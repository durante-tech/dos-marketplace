...
description: Generate comprehensive U.S. economic overview with multi-timeframe trend analysis across all 68 Substrate metrics.
status: STABLE
bestPath:
  - title: "Verify Data Availability"
    description: "Confirm the Substrate US-Common-Metrics dataset exists (run UpdateData first if not)."
  - title: "Load & Fetch Metrics"
    description: "Load metric definitions and fetch 10+ years of historical data for the 68 indicators."
  - title: "Trend & Correlation Analysis"
    description: "Calculate multi-timeframe (10y/5y/2y/1y) trends and cross-category correlations."
  - title: "Pattern Detection & Recommendations"
    description: "Identify regime changes, divergences, and extremes; surface research recommendations."
  - title: "Compile Report"
    description: "Generate the structured markdown US Economic State Analysis document."
divergence_from_canonical:
  _workflow-output-shape.md:
    partial_version: 1.0.0
    reason: "USMetrics GetCurrentState workflow has bespoke Output section with workflow-specific shape"
---
...
- User says "how is the economy?", "economic overview", "get current state", "US metrics analysis"
- Substrate US-Common-Metrics data is already current (run UpdateData first if not)
- NOT for fetching fresh data from FRED/EIA/Treasury (use UpdateData) — this workflow only reads and analyzes already-fetched data
...
### Step 1b: Verify Data Directory Exists

Before loading metrics, verify the Substrate dataset directory is present:

```
~/.claude/DOS/data/US-Common-Metrics/
```

If the directory does NOT exist, STOP and emit this message to the principal:

```
US-Common-Metrics data not yet fetched.
Run the UpdateData workflow first (requires FRED_API_KEY and EIA_API_KEY environment variables).

Quick start:
  1. export FRED_API_KEY=...   # https://fred.stlouisfed.org/docs/api/api_key.html
  2. export EIA_API_KEY=...    # https://www.eia.gov/opendata/register.php
  3. /USMetrics UpdateData
  4. /USMetrics GetCurrentState
```

Do not proceed to Step 2 until the data directory is populated.
