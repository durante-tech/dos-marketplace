 ...
 * - May update: DOS/USER/ABOUT_DANIEL.md (significant learnings)
 *
 ...
 *
 * EXAMPLES:
 * - W @Principal: Currently focused on DOS infrastructure improvements
...
function readTranscriptEntries(path: string): TranscriptEntry[] {
  ...
  try {
    ...
    // User-side signals (preferences, frustration) drive relationship analysis.
    // `parsed.userPrompt` filters out tool_result entries — see TranscriptParser.
 ...
 */
function ensureRelationshipDir(dosDir: string): string {
  const { year, month, day } = getPSTComponents();
  const monthDir = join(dosDir, 'MEMORY', 'RELATIONSHIP', `${year}-${month}`);
...
`;
  ...
  try {
    // 'wx' is O_CREAT|O_EXCL: create atomically, or fail with EEXIST. The existsSync
    // check above is only a cheap fast path — it cannot be the guard.
    //
    // The old `existsSync(...) ? return : writeFileSync(...)` was a TOCTOU. Two
    // SessionEnd hooks ending together on the day's FIRST note both observed "absent";
    // the first wrote the header and appended its notes; the second then wrote the
    // header again — and writeFileSync TRUNCATES — silently destroying the first
    // session's notes. No error, no log. A fleet of concurrent sessions makes that race
    // routine rather than theoretical. (Forge H-102.)
    writeFileSync(filepath, header, { encoding: 'utf-8', flag: 'wx' });
  } catch (err) {
    // Someone else created it between our check and our write. That is the race this
    // guard exists to lose safely: their header is as good as ours, and our notes will
    // append after it. Anything else (EACCES, ENOSPC) is a real error — let it surface.
...
async function main() {
  try {
    ...
    const filepath = ensureRelationshipDir(dosDir);
...
const _t = startTimer('RelationshipMemory');
