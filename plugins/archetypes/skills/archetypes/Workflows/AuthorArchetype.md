...
- Tier by the **in-app cohort** (T1 ≥6/8; T2 4-5/8; T3 ≤3/8 including 0 — bands are exclusive, every count maps to exactly one tier), with ONE unconditional override: a capability universal across the services cohort is T1 regardless of in-app count (in-app UIs hide backend capabilities; demoting it requires a groundingException — the validator enforces this). **A partially-inferred count is judged on its confirmed sub-count for T1 ONLY (gen-38, scope clarified gen-55/H31):** an inferred in-app count clears the T1 band only if `confirmed ≥ 6`; an inferred universal count grants the override only if `confirmed == of`. A row denied T1 by its confirmed sub-count takes its full-count band capped at T2; T2/T3 assignment always judges the flagged FULL count, never the confirmed sub-count (both gen-54 blind-derivation mismatches were band(confirmed) over-application). Alternatively the row carries a declared groundingException (the `t1-grounding` check enforces this).
...
- **Mechanical close (gen-56, H32):** before Step 4, run `bun Tools/MintAudit.ts --mint-dir <mint-record dir>` (the dir holding `synthesis.json` + the miner `*table.md` files). Zero UNMATCHED lines is the bar (`--strict` enforces it) — every unmatched line is a row or a ledger entry you owe. Adjudicate every `INFERRED?` candidate: markers about non-counted members wave through; a marker on a COUNTED member becomes `inferred: true` + `confirmed` on the row. Prose contracts don't police prose — this audit is the mechanical owner of the two classes the gen-54 driver-synthesis itself dropped (1 silent absence, 4 inferred-flags).
