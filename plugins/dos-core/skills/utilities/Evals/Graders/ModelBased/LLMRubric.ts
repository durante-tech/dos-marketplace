...
import { inference, type InferenceLevel } from '../../../DOS/Tools/Inference';
...
Evaluate the output against the rubric and provide your assessment.`;
  ...
  private parseResponse(
    ...
    // Extract score. Tolerate markdown emphasis around the value —
    // e.g. `**SCORE:** 4` or `SCORE: **4**` — which a judge model often emits
    // (UTIL-08 in DEFECT-REGISTRY-2026-07-03: bolded scores were masked as 0).
    let score = 0;
    const scoreMatch = text.match(/SCORE:[\s*_`]*(\d+(?:\.\d+)?|PASS|FAIL)/i);
