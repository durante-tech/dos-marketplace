---
name: Upgrade
description: Generate prioritized DOS upgrade recommendations via parallel context and source analysis.
status: STABLE
---
...
1. **Thread 1:** Analyze user context (TELOS, projects, recent work, DOS state)
...
"Analyze the current state of the user's DOS system:
...
Return format for EACH technique:
{
  ...
  'current_pai_gap': '[What DOS currently does/lacks that this addresses]',
  'implementation_file': '[Path to DOS file this would change]',
...
1. Load channel config:
   bun ~/.claude/DOS/Tools/LoadSkillConfig.ts ../youtube-channels.json
...
4. For NEW videos, extract transcripts:
   bun ~/.claude/DOS/Tools/GetTranscript.ts '<video-url>'
...
5. CRITICAL - For each transcript, extract SPECIFIC TECHNIQUES:
   ...
   - Identify what DOS component this applies to
...
Return format for EACH technique found:
{
  ...
  'pai_relevance': '[Which DOS component this could improve]'
}
...
1. Look in ~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/DOSUpgrade/
...
"Discover trending GitHub projects relevant to DOS for inspiration.

1. Load the github_trending config from user-sources.json:
   Read ~/.claude/DOS/USER/SKILLCUSTOMIZATIONS/DOSUpgrade/user-sources.json
...
6. For EACH new repo found, evaluate DOS relevance:
   - Read the repo's README (first 500 lines): gh api 'repos/OWNER/REPO/readme' --jq '.content' | base64 -d | head -500
   - Assess: Does this repo solve a problem DOS has? Does it use patterns DOS could adopt?
...
7. Return format for EACH relevant project:
   {
     ...
     'topics': ['[topics]'],
     'created': '[date]',
     'pai_relevance': '[1-2 sentences: what problem this solves that DOS cares about]',
     'inspiration_techniques': [
       {
         'technique': '[specific pattern, architecture, or approach worth noting]',
         'applies_to': '[which DOS component could benefit]'
       }
     ]
   }
...
- Connect findings to specific DOS components that could benefit.
...
2. SKIP rows where `tags` includes `"backfill"` — these are historical R32-parity placeholders (one-time 2026-05-18 operator-authorized bulk-backfill, identifiable by `backfill_provenance` field and `tags: ["backfill","r32-parity","historical"]`). Including them poisons frequency counts and theme clustering; the SessionEnd capture hook is working for real reflections, the backfills are pre-discipline historical filler.
3. Prioritize entries with implied_sentiment <= 5, within_budget: false, or criteria_failed > 0
4. Cluster Q2 answers (algorithm improvements) into themes by similarity
5. Cluster Q1 answers (execution patterns) into themes
6. For themes with 2+ occurrences (or 1 if sentiment <= 4), create upgrade candidates
...
> **Canonical implementation:** the scoring formula and tier banding below are implemented as deterministic code in `Workflows/dos-upgrade-scan.workflow.js` (deployed to `~/.claude/workflows/dos-upgrade-scan.js`). The code is authoritative; this prose mirrors it — if they ever disagree, fix the prose.

### Step 6: Generate Prioritized Recommendations

Sort by priority score and categorize into FOUR tiers:

- **🔴 CRITICAL:** Score > 30 AND relevance ≥ 8. Fixes gaps, security issues, or unlocks capabilities DOS should already have. Integrate immediately.
- **🟠 HIGH:** Score ≥ 22 AND relevance ≥ 6 (and not CRITICAL). Significantly improves DOS capabilities or efficiency. Integrate this week.
- **🟡 MEDIUM:** Score ≥ 14 AND relevance ≥ 4 (and not higher). Adds useful capabilities or aligns with ecosystem best practices. Integrate when convenient.
- **🟢 LOW:** Everything else that survived the relevance ≥ 3 filter. Nice-to-know, future reference, or will become relevant later.

(Relevance gates are inclusive `≥` so boundary values 8/6/4 land in the intuitive tier — matches `dos-upgrade-scan.workflow.js` review #4.)

For each recommendation, include:
- Short action name (what to do)
- DOS Relevance (WHY it matters for our system — this is the primary framing, not an afterthought)
- Effort estimate (Low/Med/High)
- Files affected (specific DOS files that would change)
...
1. **✨ Discoveries** — Everything interesting found, ranked by coolness/interestingness, with source and DOS relevance. This is the "what's out there" overview {PRINCIPAL.NAME} reads first.
...
|---|-----------|--------|---------------------|---------------|
| 1 | [Most interesting thing] | [source] | [What makes this cool — 1-2 sentences] | [How it maps to DOS] |
| 2 | [Next most interesting] | [source] | [Why it's notable] | [DOS connection] |
| ... | ... | ... | ... | ... |
...
[Fixes gaps, security issues, or unlocks capabilities DOS should already have]

| # | Recommendation | DOS Relevance | Effort | Files Affected |
|---|---------------|---------------|--------|----------------|
| [N] | [Short action] | [Why DOS needs this NOW] | [Low/Med/High] | `[files]` |

### 🟠 HIGH — Integrate this week
[Significantly improves DOS capabilities or efficiency]
...
[Describe the specific DOS benefit. Must be 16-32 words.]
...
**How It Helps DOS (16-32 words):**
[Describe which DOS component could adopt this pattern and what improvement it would bring.]

**Inspiration Techniques:**
> [Specific architectural pattern, approach, or code technique worth noting from the repo's README or code]

**Applies To:** `[DOS file path]`, [component name]
**Potential Integration:**
[How DOS could adopt this pattern — not a full implementation, but the key insight to borrow]
...
2. **DISCOVERIES ≠ RECOMMENDATIONS** - The Discoveries table ranks by "how cool is this" (interestingness). The Recommendations section ranks by "how urgently should DOS integrate this" (priority). These are DIFFERENT orderings. A LOW-priority item can be Discovery #1 if it's the most interesting thing found.
3. **RECOMMENDATIONS COME SECOND** - The 🔥 Recommendations section with four priority tiers follows the Discoveries table.
4. **DOS RELEVANCE IS PRIMARY** - In BOTH the Discoveries table AND the Recommendation tables, explain WHY this matters for DOS.
...
8. **MAP TO DOS** - Every technique must name a specific DOS file or component it improves
9. **TWO MANDATORY DESCRIPTIONS (16-32 words each):**
   ...
   - **How It Helps DOS:** Describe the specific benefit - which component improves, what gap it fills
...
- `State/github-trending.json` - Add newly seen repo full_names (runtime-created)
...
| # | Recommendation | DOS Relevance | Effort | Files Affected |
|---|---------------|---------------|--------|----------------|
| 2 | Replace session ID hacks with native ${CLAUDE_SESSION_ID} | Session documentation workflows have manual extraction workarounds — native variable eliminates fragile code | Low | system session-documentation workflow |
...
**How It Helps DOS (16-32 words):**
Our session documentation workflows had manual session ID extraction hacks. Native substitution means cleaner code and reliable session tracking across all DOS workflows.
...
**Applies To:** system session-documentation workflow
...
| SDK update v0.78 | GitHub | DOS uses CLI, not raw SDK |

## 🔍 Sources Processed
24 Anthropic sources, 5 YouTube videos, 0 custom → 2 relevant findings
```

---

**This workflow implements the core DOSUpgrade value proposition: understanding YOU first, discovering what's new second, then connecting them into actionable, personalized upgrades.**
