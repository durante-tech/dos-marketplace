...
const TAB_TITLES_DIR = dosPath('MEMORY', 'STATE', 'tab-titles');
const KITTY_SESSIONS_DIR = dosPath('MEMORY', 'STATE', 'kitty-sessions');

function runKitten(args: string[]): boolean {
  const result = spawnSync('kitten', ['@', ...args], {
    stdio: 'ignore',
    timeout: 2000,
  });
...
function runKittenOutput(args: string[]): string {
  const result = spawnSync('kitten', ['@', ...args], {
    encoding: 'utf-8',
...
function cleanupStaleStateFiles(): void {
  try {
    ...
    const liveOutput = runKittenOutput([`--to=${socketPath}`, 'ls']).trim();
    if (!liveOutput) return;

    const liveIds = new Set<string>();
    const kittyState = JSON.parse(liveOutput) as Array<{
      tabs?: Array<{ windows?: Array<{ id?: string | number }> }>;
    }>;
    for (const osWindow of kittyState) {
      for (const tab of osWindow.tabs ?? []) {
        for (const window of tab.windows ?? []) {
          if (window.id !== undefined) liveIds.add(String(window.id));
        }
      }
    }
...
export function setTabState(opts: SetTabOptions): void {
  ...
  try {
    ...
    runKitten([toFlag, 'set-tab-title', title]);
    runKitten([toFlag, 'set-window-title', title]);

    // For idle state, reset ALL colors to Kitty defaults (no lingering backgrounds)
    if (state === 'idle') {
      runKitten([toFlag, 'set-tab-color', '--self', 'active_bg=none', 'active_fg=none', 'inactive_bg=none', 'inactive_fg=none']);
    } else {
      runKitten([toFlag, 'set-tab-color', '--self', `active_bg=${ACTIVE_TAB_BG}`, `active_fg=${ACTIVE_TAB_FG}`, `inactive_bg=${colors.inactiveBg}`, `inactive_fg=${INACTIVE_TAB_FG}`]);
    }
...
export function setPhaseTab(phase: AlgorithmTabPhase, sessionId: string, summary?: string): void {
  ...
  try {
    ...
    if (phase === 'IDLE') {
      ...
      runKitten([toFlag, 'set-tab-color', '--self', `active_bg=${ACTIVE_TAB_BG}`, `active_fg=${ACTIVE_TAB_FG}`, `inactive_bg=${config.inactiveBg}`, `inactive_fg=${INACTIVE_TAB_FG}`]);
    }
