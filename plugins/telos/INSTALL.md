...
# Check for DOS infrastructure
if [ -d "$CLAUDE_DIR/DOS" ]; then
  echo "OK DOS directory exists"
else
  echo "INFO DOS directory not found (personal TELOS features will be limited)"
fi

# Check for personal TELOS files
if [ -d "$HOME/.durante/user/TELOS" ]; then
  echo "OK Personal TELOS directory exists at: $HOME/.durante/user/TELOS"
  ls "$HOME/.durante/user/TELOS/"*.md 2>/dev/null | wc -l | xargs echo "  TELOS files found:"
else
  echo "INFO Personal TELOS directory not found (personal context features unavailable)"
  echo "  Personal TELOS is created during DOS setup, not by this skill pack"
fi
...
if [ -d "$CLAUDE_DIR/DOS/USER/SKILLCUSTOMIZATIONS" ]; then
...
are created during DOS setup, not by this skill pack. The telos skill will work for
project analysis without them. For personal life context features, set up DOS first:
...
Personal TELOS files at ~/.durante/user/TELOS/ will NOT be touched."
```
...
at ~/.durante/user/TELOS/. These are created during DOS setup:
https://github.com/durante-tech/dos
...
Personal TELOS files live at `~/.durante/user/TELOS/`, which is created during DOS setup -- not by this skill pack. The telos skill works for project analysis without personal TELOS files.
